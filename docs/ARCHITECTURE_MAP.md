...
- `services/SalesPipelineService.php` — canonical per-lead business sales state, stage history, tags, outcome and sale facts; independent from technical conversation status. Assigned inactive tags remain preserved/readable facts while only active catalog tags may be newly selected or replaced.
...
- `services/ManagerPushService.php` + `services/ManagerPushHealth.php` — canonical Manager browser-push delivery and health owners. Delivery records endpoint health; health exposes usability separately from manager shift state. Browser repair must consume this health rather than invent reachability or shift semantics.
- `docs/handoff-policy-inventory.json` + `tools/handoff_policy_inventory.php` — read-only caller and policy-surface inventory for manager handoff. It guards the current working-hours, availability, presentation, queue mutation and five-minute fallback ownership without changing those policies or runtime behavior.
- `services/ChannelOfferService.php` — canonical pre-results channel-promotion owner. Suppression is an explicit source policy (`conversation_sources.suppress_channel_offer`) matched by the attributed source key; missing/unknown sources fail open to the configured offers, and transport alone must never imply suppression.
...
- `manager/assets/workspace-v2-pipeline.js` — Workspace V2 browser owner for sales-stage/tag/outcome mutations plus pipeline filters; lead-card rendering delegates mutation wiring here instead of issuing sales writes directly. Disabled inactive assigned tags are presentation facts and must not enter mutation payloads.
- `manager/assets/workspace-v2-notifications.js` — Workspace V2 browser owner for push setup/repair and notification-health presentation. It may recreate a backend-confirmed unhealthy browser subscription but must never mutate or infer manager shift/routing state.
...
- `services/RoutingAdminService.php` — canonical admin mutation/read owner for source routing policy, including channel-promotion suppression; UI and transport adapters pass policy values but do not infer them.
- `manager/routing.php` + `manager/assets/routing.css` + `manager/assets/routing.js` — current role-gated routing interface; PHP remains a thin shell and routing/source policy is not to be duplicated in frontend code.
...
- Any Manager browser-push setup/repair behavior outside `workspace-v2-notifications.js` → converge there; delivery truth remains in `ManagerPushService`/`ManagerPushHealth` and manager shift state remains system-owned elsewhere.
