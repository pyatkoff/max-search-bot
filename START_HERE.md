...
The next safe item is a read-only country-flow contract inventory only. Trace `pick_country_*` callbacks and the `statusContryChoose` free-text branch; exact country-directory ID representation; popular and manual-country payloads; unknown short input and rich-request AI fallback; edit/back behavior; missing/pre-start semantics; AI normalization/default ownership; and the downstream `UF_COUNTRY` → search `country` projection. Record current behavior and ownership without changing runtime. Do not change callback payloads, directory queries, stored values, AI normalization/defaults, URL/Tourvisor projection, country/date mutations, or any protected business mechanism. No country runtime migration is authorized until the inventory is production-green and a separate executable value/application contract plus explicit checkpoint proves exact semantics.
