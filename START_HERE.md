...
- The disconnected side-effect-free `DateValueContract` shipped production-green in PR #709. It preserves exact calendar-valid `DD.MM.YYYY`, parses only `pick_date_DD.MM.YYYY`, does not own today/future policy and remains disconnected from runtime. Do not repeat it.
...
The date-flow inventory and disconnected value contract are complete in PRs #707 and #709 and must not be repeated. The next safe item is one guarded date-selection callback-only update slice. In `WizardCallbackAction::handleDateSelection`, project only `pick_date_DD.MM.YYYY` through `DateValueContract::fromCallbackPayload`, then apply the exact returned string through `ExistingWizardStepApplicationService::apply` for `statusDate` and check its result before edit/normal progression. Preserve the existing `InteractionGuard::runExpectedStatusCallback` `date_selection` scope, expected `statusDate`, stale/concurrent consumption and diagnostics, normal/edit return to `check`, calendar payloads and today/future presentation policy. Invalid payloads and missing or pre-start date steps must fail closed without insertion, mutation or progression; add a real-action regression for valid past/present/future representation, leap day, malformed payloads, missing/pre-start rows, stale and duplicate delivery, and edit completion. Do not change month navigation, wizard free text, AI/pending-month/date-parser behavior, calendar UX, stored representation, URL/Tourvisor, Yandex Metrica/goals, lead delivery, manager shifts/routing, AI-log boundary or deploy provenance. No wizard free-text or AI date migration is authorized until this callback slice is production-green and another explicit checkpoint is merged.
