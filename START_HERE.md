...
The departure-city inventory, value contract and guarded callback slice are complete in PRs #691, #693 and #695 and must not be repeated. Only one runtime caller is now authorized: the existing departure-city free-text branch in `StateMessageHandler`. Preserve its lookup order exactly: `MaxSearchApi::getCityByName()` first, then `DepartureCityResolver::resolveFieldValue()` only on a miss. Normalize the resolved directory ID through `DepartureCityValueContract::fromDirectoryId()` and apply the exact decimal string through `ExistingWizardStepApplicationService::apply()` to the already-existing city step. A rejected ID or failed application, including a missing or pre-start city row, must return fail-closed without value mutation, next-view rendering, error text or status progression. Preserve the short unknown-city validation message, rich-request routing to `AiMessageHandler`, normal country progression and city edit return to check. Required executable real-handler tests must cover a primary directory hit, resolver fallback hit, no-flight ID `99`, unknown short input, rich input, missing and pre-start rows, normal flow and edit flow; they must also prove neither lookup order nor AI/default behavior changed. Keep `DepartureCityResolver::resolveAndStore`, `NeedApplicationService`, AI normalization/upsert/defaults, directory queries, callback payloads, URL/Tourvisor projection, country and date mutations out of scope. No later caller is authorized until this free-text slice is production-green and another explicit checkpoint is merged.
