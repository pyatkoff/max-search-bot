...
The next autonomous runtime slice is only the `statusAge` free-text branch in `StateMessageHandler`. Parse with `ChildAgeValueContract::parseLegacyInput`, project with `ChildAgeValueContract::toStorage`, then apply the exact string through `ExistingWizardStepApplicationService::apply`. Preserve every accepted and rejected separator case from PR #687, exact child-count validation, the `0..17` age range, current error messages, tourists edit return and normal progression to stars. Fail closed without rendering or transitioning when the existing age step is missing. Required executable cases are one age, zero age, space, comma, comma-space, current mixed-separator edge behavior, each rejected separator, wrong count, out of range, missing step, normal progression and tourists edit. Do not change `NeedValueResolver`, AI completion paths, other free-text fields or callback flows in that PR. Keep city, country and date mutations deferred, and do not make observe-only transition validation blocking, without their own explicit contracts and clean production evidence.
