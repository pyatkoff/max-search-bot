...
- The side-effect-free `DateValueContract` shipped production-green in PR #709. It preserves exact calendar-valid `DD.MM.YYYY`, parses only `pick_date_DD.MM.YYYY` and does not own today/future policy. The guarded date-selection callback update-only slice shipped production-green in PR #711 with malformed, missing/pre-start, stale/duplicate and edit coverage. Do not repeat either slice.
...
The date-flow inventory, value contract and guarded date-selection callback update-only slice are complete in PRs #707, #709 and #711 and must not be repeated. The next safe item is one wizard free-text date update-only slice in the existing `statusDate` branch of `StateMessageHandler`. Preserve the exact resolution order: `AiDateHandler::resolvePendingShortDate` first, `AiDateHandler::rememberMonthFromText` second, month-only calendar rendering without a date write, and the current invalid-input message. For a nonempty resolved date, project it through `DateValueContract::fromStorageValue`, then apply the exact returned string through `ExistingWizardStepApplicationService::apply` for `statusDate` and check its result before edit/normal progression. Do not add an interaction guard to this message path. Preserve valid past/present/future and leap-day representation, pending short-date context, normal/edit return to `check`, and fail closed without insertion, mutation or progression when the date step is missing or pre-start. Add a real-handler regression for full date, pending short date, month-only, invalid input, valid past/present/future and leap day, missing/pre-start rows and edit completion. Do not change date parsing or today/future policy, callback/month-navigation behavior, AI application/pending-month ownership, calendar UX, stored representation, URL/Tourvisor, Yandex Metrica/goals, lead delivery, manager shifts/routing, AI-log boundary or deploy provenance. No AI date migration is authorized until this free-text slice is production-green and another explicit checkpoint is merged.
