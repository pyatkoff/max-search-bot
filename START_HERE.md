...
No additional runtime slice is currently authorized. The read-only departure-city contract inventory is complete in PR #691 and must not be repeated. The next safe item is an executable contract/tests-only PR for departure-city ID normalization: add a side-effect-free `DepartureCityValueContract` that projects positive directory IDs, including no-flight ID `99`, to the exact decimal-string wizard storage representation and parses only the existing `pick_city_<id>` payload shape. Cover the fixed button IDs `1`, `5`, `10`, `12`, `99`, another positive directory ID, zero, negative, fractional, empty and non-numeric inputs. Keep the contract disconnected from `WizardCallbackAction`, `StateMessageHandler`, AI application and all runtime writers. Do not change callback payloads, directory lookup, URL/Tourvisor values or behavior. A later runtime PR remains blocked until this contract is production-green, missing/pre-start city-step behavior is explicitly fail-closed, and a separate START_HERE checkpoint authorizes one caller. Keep country and date mutations deferred, and do not make observe-only transition validation blocking, until each has its own explicit contract and clean production evidence.
