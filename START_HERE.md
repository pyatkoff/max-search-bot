...
- The side-effect-free `DepartureCityValueContract` shipped production-green in PR #693. It projects positive canonical directory IDs, including no-flight `99`, to exact decimal-string wizard storage and parses only the existing `pick_city_<id>` shape. It remains disconnected from runtime; do not repeat the contract slice.
...
The departure-city inventory and disconnected value contract are complete in PRs #691 and #693 and must not be repeated. Only one runtime caller is now authorized: a callback-only `pick_city_<id>` slice in `WizardCallbackAction`. Parse the payload through `DepartureCityValueContract::fromCallbackPayload()` and apply the resulting exact decimal string through `ExistingWizardStepApplicationService::apply()` to the already-existing city step. A rejected payload or failed application, including a missing or pre-start city step, must be consumed fail-closed without value mutation, next-view rendering or status progression. Preserve the existing `pick_city_other` manual path, `back_pick_country`, `InteractionGuard` `wizard.forward` locking, stale/duplicate/concurrent suppression, normal country progression and edit return to check. Required executable tests must cover fixed IDs `1`, `5`, `10`, `12`, no-flight `99`, another positive ID, invalid payload rejection, duplicate/stale delivery, both missing and pre-start rows, normal flow and city edit. Keep `StateMessageHandler`, `DepartureCityResolver`, AI application, directory lookup, callback payloads, URL/Tourvisor projection, country and date mutations out of scope. No later caller is authorized until this callback slice is production-green and another explicit checkpoint is merged.
