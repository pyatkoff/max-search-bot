...
No additional runtime slice is currently authorized. The next safe item is a read-only departure-city contract inventory only: trace `pick_city_*` callbacks and the `statusCityChoose` free-text branch, the exact stored directory ID representation, the special no-flight ID, manual-city fallback, AI fallback, edit/back behavior and missing-step semantics. Do not change callback payloads, directory lookup, URL/Tourvisor values or runtime behavior in that inventory PR. Keep country and date mutations deferred, and do not make observe-only transition validation blocking, until each has its own explicit contract and clean production evidence.
