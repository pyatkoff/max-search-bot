...
The date-flow inventory is complete in PR #707 and must not be repeated. The next safe item is one side-effect-free executable `DateValueContract` only. It must preserve the exact `DD.MM.YYYY` storage string for calendar-valid dates and parse only the existing `pick_date_DD.MM.YYYY` callback shape. This representational contract must not choose or change today/future policy: cover valid present, past and future calendar dates, leap day, invalid day/month/leap dates, missing leading zeroes, ISO form, whitespace, time/extra suffixes, empty, non-string, boolean and null values. Keep it disconnected from `WizardCallbackAction`, `StateMessageHandler`, `AiDateContextService`, `NeedApplicationService`, `NativeDateService` and every runtime writer. It must have no messenger, storage, parser, calendar, URL/Tourvisor or protected-business dependency. No date callback, wizard free-text or AI runtime migration is authorized until this contract is production-green and another explicit checkpoint is merged; the existing path-specific validation, pending-month, guard, edit and progression behavior remains frozen.
