...
The country-flow inventory, value contract and guarded callback-only update slice are complete in PRs #699, #701 and #703 and must not be repeated. The next safe item is one country free-text update-only slice. Preserve the existing exact active-directory lookup through `MaxSearchApi::getCountryByName` with no new resolver or fuzzy matching. When it returns a country, project its directory `ID` through `CountryValueContract::fromDirectoryId` and apply it through `ExistingWizardStepApplicationService::apply` for `statusContryChoose`; preserve exact decimal-string storage, normal progression to adults and edit return to check. Invalid directory IDs and missing or pre-start country steps must fail closed without insertion, mutation or progression. Preserve the current short unknown-country validation copy and rich-request AI routing exactly. Add a real-handler regression covering integer and PDO-string directory IDs including the popular IDs, invalid IDs, unknown short input, rich AI input, normal/edit progression and missing/pre-start rejection. Do not change directory queries, callback behavior, AI normalization/parser/hints/defaults or upsert semantics, URL/payload/Tourvisor, Yandex Metrica/goals, lead delivery, manager shifts, routing, AI-log boundary or deploy provenance. After this slice is production-green, record completion before selecting any new runtime caller.
