...
The country-flow inventory is complete in PR #699 and must not be repeated. The next safe item is one side-effect-free executable `CountryValueContract` only. It must project a positive canonical integer or decimal-string country directory ID to the exact decimal storage string and parse only the existing `pick_country_<positive canonical id>` payload shape. Cover the popular IDs `1`, `2`, `4`, `8`, `9` and `12`, another positive directory ID, and rejection of zero, negative, fractional, empty, non-numeric, leading-zero, signed, whitespace-padded, boolean, null and manual/extra-suffix payloads. The contract must have no runtime mutation, directory, AI, URL/Tourvisor or protected-business dependency and must remain disconnected from every runtime caller. No callback or free-text application is authorized until this contract is production-green and another explicit checkpoint is merged.
