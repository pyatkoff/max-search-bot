...
The country-flow inventory and disconnected value contract are complete in PRs #699 and #701 and must not be repeated. The next safe item is one guarded country callback-only update slice. Route only valid existing `pick_country_<positive canonical id>` payloads through `CountryValueContract::fromCallbackPayload` and `ExistingWizardStepApplicationService::apply` for `statusContryChoose`; preserve exact decimal-string storage, the existing `country_selected` funnel event, normal progression to adults and edit return to check. Missing or pre-start country steps, invalid payloads, stale callbacks and duplicate callbacks must be consumed without insertion, mutation or progression. Preserve `pick_country_other`, back behavior, `InteractionGuard` serialization and all existing payload IDs exactly. Add a real-handler regression covering IDs `1`, `2`, `4`, `8`, `9`, `12`, another positive ID, edit, manual, invalid, missing/pre-start, stale and duplicate cases. Country free text, directory lookup, AI normalization/defaults, URL/Tourvisor and every protected mechanism remain out of scope. Do not start a country free-text runtime slice until this callback slice is production-green and another explicit checkpoint is merged.
