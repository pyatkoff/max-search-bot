...
ctxCheck('legacy deployment keeps manager cookie under max-search',ManagerRequestContext::sessionCookiePath('/max-search/manager/index.php')==='/max-search/manager/');
ctxCheck('app subdomain deployment scopes cookie to root manager path',ManagerRequestContext::sessionCookiePath('/manager/index.php')==='/manager/');
ctxCheck('nested manager API keeps canonical manager cookie path',ManagerRequestContext::sessionCookiePath('/manager/api/conversation.php')==='/manager/');
ctxCheck('missing script name falls back to root manager path',ManagerRequestContext::sessionCookiePath('')==='/manager/');
...
$client=(string)file_get_contents(dirname(__DIR__).'/manager/assets/manager-http-client.js');
ctxCheck('session cookie policy stays centralized and deployment-aware',strpos($source,"'path' => self::sessionCookiePath()")!==false&&strpos($source,"'secure' => true")!==false&&strpos($source,"'httponly' => true")!==false&&strpos($source,"'samesite' => 'Lax'")!==false);
...
ctxCheck('admin and routing share one frontend API transport owner',strpos($client,"endpoint='api.php'")!==false&&strpos($client,'fetch(endpoint')!==false&&strpos($client,"credentials:'same-origin'")!==false&&strpos($client,"error:'network_error'")!==false&&strpos($client,"error:'invalid_response'")!==false&&strpos($adminJs,'ManagerHttpClient.request(action,data,S.csrf)')!==false&&strpos($routingJs,'ManagerHttpClient.request(action,data,S.csrf)')!==false&&strpos($adminJs,"fetch('api.php'")===false&&strpos($routingJs,"fetch('api.php'")===false);
ctxCheck('shared frontend client loads before admin and routing modules',strpos($admin,'assets/manager-http-client.js?v=')<strpos($admin,'assets/admin.js?v=')&&strpos($routing,'assets/manager-http-client.js?v=')<strpos($routing,'assets/routing.js?v='));
