...
stableCheck('manager index is redirect-free and renders one workspace implementation',strpos($index,"header('Location:")===false&&strpos($index,"require __DIR__.'/workspace-v2.php'")!==false);
stableCheck('manager aliases canonicalize without network navigation',strpos($core,'history.replaceState')!==false&&strpos($core,"location.replace(")===false&&strpos($core,'(?:index|workspace-v2)\\.php')!==false);
