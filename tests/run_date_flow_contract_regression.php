...
$dateStart = strpos($handler, 'elseif($status==MaxSearchApi::$statusDate)');
$phoneStart = $dateStart === false ? false : strpos($handler, 'elseif($status==MaxSearchApi::$statusPhone)', $dateStart);
$dateBranch = ($dateStart !== false && $phoneStart !== false) ? substr($handler, $dateStart, $phoneStart - $dateStart) : '';
$failed = 0;
...
dateContractCheck('technical lead window is not customer flexibility', ($contract['state']['flexibility_policy'] ?? null) === 'customer_date_flexibility_is_unknown_unless_explicitly_supplied_and_must_not_be_inferred_from_the_technical_plus_or_minus_3_day_window');
dateContractCheck('customer confirmation and search use exact selected date while lead window stays separate', ($contract['state']['search_window'] ?? null) === 'customer_confirmation_and_search_query_use_the_single_selected_date;_the_existing_lead_window_remains_a_separate_plus_or_minus_3_day_technical_behavior');
dateContractCheck('callback and wizard-connected value contract owns representation without today/future policy', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DateValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === 'date_selection_callback_and_wizard_free_text' && ($contract['executable_value_contract']['storage_projection'] ?? null) === 'calendar_valid_exact_DD.MM.YYYY_string_preserved_verbatim' && ($contract['executable_value_contract']['today_or_future_policy'] ?? null) === 'intentionally_not_owned');
...
dateContractCheck('date selection uses validated checked update-only application', strpos($callback, 'DateValueContract::fromCallbackPayload($q)') !== false && strpos($callback, 'ExistingWizardStepApplicationService::apply(') !== false && strpos($callback, "MaxSearchApi::saveLastValue(\$chatId, MaxSearchApi::\$statusDate, str_replace('pick_date_', '', \$q));") === false && ($contract['selection_callback']['application'] ?? null) === 'ExistingWizardStepApplicationService::apply' && ($contract['selection_callback']['application_semantics'] ?? null) === 'update_existing_current_session_statusDate_row_only' && ($contract['selection_callback']['application_result_checked'] ?? null) === true && ($contract['selection_callback']['payload_validation_at_action_boundary'] ?? null) === 'DateValueContract::fromCallbackPayload' && ($contract['selection_callback']['missing_or_pre_start_behavior'] ?? null) === 'consume_without_mutation_or_progression');
...
dateContractCheck('wizard recognized date uses canonical resolver/application and checked update-only application', $dateBranch !== '' && strpos($dateBranch, 'NeedApplicationService::resolveAndApplyExistingWizardStep(') !== false && strpos($dateBranch, "'date'") !== false && strpos($dateBranch, '(int)MaxSearchApi::$statusDate') !== false && strpos($dateBranch, 'DateValueContract::fromStorageValue($date)') === false && strpos($dateBranch, 'ExistingWizardStepApplicationService::apply(') === false && strpos($dateBranch, 'MaxSearchApi::saveLastValue($chat_id, MaxSearchApi::$statusDate, $date);') === false && ($contract['wizard_free_text']['recognized_date_application'] ?? null) === 'NeedApplicationService::resolveAndApplyExistingWizardStep' && ($contract['wizard_free_text']['application_semantics'] ?? null) === 'resolve_date_via_NeedValueResolver_then_update_existing_current_session_statusDate_row_only' && ($contract['wizard_free_text']['application_result_checked'] ?? null) === true && ($contract['wizard_free_text']['missing_or_pre_start_behavior'] ?? null) === 'consume_without_mutation_or_progression' && array_key_exists('interaction_guard_scope', $contract['wizard_free_text']) && $contract['wizard_free_text']['interaction_guard_scope'] === null);
...
dateContractCheck('legacy direct date helper retains auto-insert fallback outside migrated wizard paths', strpos($maxSearch, 'in_array($status,[static::$statusAge,static::$statusDate],true)&&static::getLastValue($chatID,$status)===false') !== false);
...
dateContractCheck('calendar copy keeps exact search date and makes flexibility separate', strpos($view, 'Поиск откроется на эту дату; если даты можно сдвигать, напишите об этом отдельно.') !== false && ($contract['calendar']['customer_copy'] ?? null) === 'selected_date_is_the_exact_search_date;_date_flexibility_must_be_supplied_separately');
...
dateContractCheck('summary mirrors the exact selected search date without inventing flexibility', strpos($summary, "' · поиск на эту дату'") !== false && strpos($summary, "modify('-3 days')") === false && ($contract['downstream']['summary'] ?? null) === 'SearchDateSummary_formats_the_selected_date_and_states_that_search_uses_that_date_without_inventing_customer_flexibility');
dateContractCheck('legacy lead window remains separate and is not treated as customer flexibility', ($contract['downstream']['lead_window'] ?? null) === 'NativeDateService_leadWindow_remains_plus_or_minus_3_days_as_existing_lead_behavior_and_is_not_evidence_of_customer_flexibility');
dateContractCheck('trip state keeps exact date but does not invent flexibility', strpos($tripState, "'dates'=>['from'=>\$date,'to'=>\$date,'month'=>self::monthFromDate(\$date),'flexible_days'=>null]") !== false && ($contract['downstream']['trip_state'] ?? null) === 'dates.from_and_dates.to_equal_stored_value_dates.month_is_YYYY-MM_and_flexible_days_is_null_until_explicitly_supplied');

$protected = (array)($contract['protected_non_goals'] ?? []);
dateContractCheck('wizard migration does not authorize AI date runtime', ($contract['migration_readiness']['runtime_migration_allowed'] ?? null) === false && ($contract['migration_readiness']['reason'] ?? null) === 'date_selection_callback_and_wizard_free_text_use_update_only_contract_while_AI_retains_separate_upsert_and_future_policy' && ($contract['migration_readiness']['required_before_runtime'] ?? null) === ['explicit_checkpoint_before_any_AI_date_migration']);
dateContractCheck('other runtime payload calendar mechanics and protected mechanisms stay frozen', in_array('runtime behavior changes outside date selection callback and wizard free text', $protected, true) && in_array('callback payload changes', $protected, true) && in_array('date parsing changes', $protected, true) && in_array('calendar navigation, callback payload and selection mechanics', $protected, true) && in_array('URL or Tourvisor projection changes', $protected, true) && in_array('Yandex Metrica or goal changes', $protected, true) && in_array('lead delivery changes', $protected, true) && in_array('manager shift or routing changes', $protected, true) && in_array('AI log boundary changes', $protected, true) && in_array('deploy provenance changes', $protected, true));
