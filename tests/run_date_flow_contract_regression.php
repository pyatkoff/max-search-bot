...
dateContractCheck('disconnected value contract owns representation without today/future policy', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DateValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === false && ($contract['executable_value_contract']['storage_projection'] ?? null) === 'calendar_valid_exact_DD.MM.YYYY_string_preserved_verbatim' && ($contract['executable_value_contract']['today_or_future_policy'] ?? null) === 'intentionally_not_owned');
...
dateContractCheck('value contract does not authorize a runtime migration', ($contract['migration_readiness']['runtime_migration_allowed'] ?? null) === false && ($contract['migration_readiness']['reason'] ?? null) === 'date_value_contract_is_disconnected_and_direct_wizard_writes_and_AI_upsert_have_different_validation_and_missing_step_semantics' && count((array)($contract['migration_readiness']['required_before_runtime'] ?? [])) === 3);
