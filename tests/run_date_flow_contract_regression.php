...
dateContractCheck('wizard recognized date uses checked update-only application', strpos($handler, 'DateValueContract::fromStorageValue($date)') !== false && strpos($handler, 'ExistingWizardStepApplicationService::apply(') !== false && strpos($handler, 'MaxSearchApi::saveLastValue($chat_id, MaxSearchApi::$statusDate, $date);') === false && ($contract['wizard_free_text']['recognized_date_application'] ?? null) === 'ExistingWizardStepApplicationService::apply' && ($contract['wizard_free_text']['application_semantics'] ?? null) === 'update_existing_current_session_statusDate_row_only' && ($contract['wizard_free_text']['application_result_checked'] ?? null) === true && ($contract['wizard_free_text']['missing_or_pre_start_behavior'] ?? null) === 'consume_without_mutation_or_progression' && array_key_exists('interaction_guard_scope', $contract['wizard_free_text']) && $contract['wizard_free_text']['interaction_guard_scope'] === null);
