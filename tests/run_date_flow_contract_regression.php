...
dateContractCheck('callback-connected value contract owns representation without today/future policy', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DateValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === 'date_selection_callback_only' && ($contract['executable_value_contract']['storage_projection'] ?? null) === 'calendar_valid_exact_DD.MM.YYYY_string_preserved_verbatim' && ($contract['executable_value_contract']['today_or_future_policy'] ?? null) === 'intentionally_not_owned');
...
dateContractCheck('legacy direct date helper retains auto-insert fallback outside callback selection', strpos($maxSearch, 'in_array($status,[static::$statusAge,static::$statusDate],true)&&static::getLastValue($chatID,$status)===false') !== false && ($contract['wizard_free_text']['application_semantics'] ?? null) === 'direct_write_with_statusDate_auto_insert_fallback_when_no_current_session_value_row_exists');
...
dateContractCheck('callback migration does not authorize another runtime path', ($contract['migration_readiness']['runtime_migration_allowed'] ?? null) === false && ($contract['migration_readiness']['reason'] ?? null) === 'date_selection_callback_is_update_only_but_wizard_free_text_and_AI_paths_still_have_different_application_validation_and_missing_step_semantics' && ($contract['migration_readiness']['required_before_runtime'] ?? null) === ['production_green_date_selection_callback_checkpoint', 'explicit_decision_on_callback_and_wizard_free_text_validation_parity', 'executable_missing_or_pre_start_and_edit_flow_cases']);
dateContractCheck('other runtime payload calendar search and protected mechanisms stay frozen', in_array('runtime behavior changes outside date selection callback', $protected, true) && in_array('callback payload changes', $protected, true) && in_array('date parsing changes', $protected, true) && in_array('calendar UX changes', $protected, true) && in_array('URL or Tourvisor projection changes', $protected, true) && in_array('Yandex Metrica or goal changes', $protected, true) && in_array('lead delivery changes', $protected, true) && in_array('manager shift or routing changes', $protected, true) && in_array('AI log boundary changes', $protected, true) && in_array('deploy provenance changes', $protected, true));
