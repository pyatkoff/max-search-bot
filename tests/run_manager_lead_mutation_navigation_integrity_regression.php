...
navSaveCheck('failed lead refresh stays local and remains retryable',strpos($conversation,'if(stillCurrent&&!S.detail?.lead)window.WorkspaceV2LeadCard?.renderUnavailable')!==false&&strpos($lead,'function renderUnavailable(c)')!==false&&strpos($lead,'Повторить загрузку')!==false&&strpos($lead,'refreshLeadData({conversationId:Number(S.current||0)})')!==false);
