...
navSaveCheck('task create toggle and pin capture their source lead',substr_count($lead,'const target=Number(S.current||0)')>=2&&strpos($lead,"pipe('create_task',{conversation_id:target")!==false&&strpos($lead,"pipe('set_task_completed',{conversation_id:target")!==false&&strpos($lead,"pipe('set_task_pinned',{conversation_id:target")!==false);
...
navSaveCheck('task mutations keep in-flight ownership outside rerendered DOM',strpos($lead,'const taskMutations=new Map()')!==false&&strpos($lead,'async function guardTaskMutation(key,signature,work)')!==false&&strpos($lead,'taskMutations.get(key)')!==false&&strpos($lead,'taskMutations.set(key,{signature,promise:pending})')!==false&&strpos($lead,'taskMutations.delete(key)')!==false);
navSaveCheck('same task mutation payload coalesces while conflicting payload is rejected',strpos($lead,'if(active)return active.signature===signature?active.promise:false')!==false&&strpos($lead,'guardTaskMutation(`create:${target}`,signature')!==false&&strpos($lead,'guardTaskMutation(`update:${target}:${id}`,signature')!==false&&strpos($lead,'guardTaskMutation(`toggle:${target}:${id}`,String(!!completed)')!==false&&strpos($lead,'guardTaskMutation(`pin:${target}:${id}`,String(!!pinned)')!==false);
navSaveCheck('task mutation guards remain scoped by lead and task ids',strpos($lead,'`create:${target}`')!==false&&strpos($lead,'`update:${target}:${id}`')!==false&&strpos($lead,'`toggle:${target}:${id}`')!==false&&strpos($lead,'`pin:${target}:${id}`')!==false);
