...
navSaveCheck('tag saves are independently scoped per lead',strpos($pipeline,'const outcomeSavingLeads=new Set(),tagSavingLeads=new Set()')!==false&&strpos($pipeline,'tagSavingLeads.has(target)')!==false&&strpos($pipeline,'tagSavingLeads.add(target)')!==false&&strpos($pipeline,'tagSavingLeads.delete(target)')!==false);
