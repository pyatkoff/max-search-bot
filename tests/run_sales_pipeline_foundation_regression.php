...
spCheck('repair migration checks existing column',strpos($migration012,'information_schema.COLUMNS')!==false && strpos($migration012,"COLUMN_NAME='lead_stage_key'")!==false && strpos($migration012,'PREPARE lead_stage_column_stmt')!==false);
spCheck('repair migration checks existing index',strpos($migration012,'information_schema.STATISTICS')!==false && strpos($migration012,"INDEX_NAME='idx_conversations_lead_stage'")!==false && strpos($migration012,'PREPARE lead_stage_index_stmt')!==false);
spCheck('repair migration recreates missing tag storage',strpos($migration012,'CREATE TABLE IF NOT EXISTS lead_tags')!==false && strpos($migration012,'CREATE TABLE IF NOT EXISTS conversation_lead_tags')!==false);
