...
sciCheck('late send completion refreshes source only when still current and preserves newer attachments',strpos($conversation,"await open(target,{stickToBottom:true,mobileHistory:'none',preserveAttachment:true})")!==false&&strpos($conversation,"await open(S.current,{stickToBottom:true")===false);
...
sciCheck('text network exception is reported only in originating conversation',strpos($conversation,"catch(e){\n    if(!S.authExpired&&openSeq===generation&&Number(S.current)===target)setReplyStatus('Не удалось отправить сообщение','error')")!==false);
