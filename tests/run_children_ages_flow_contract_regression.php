...
childrenAgesContractCheck('child-age message migration remains blocked by projection and parser parity', empty($contract['migration_readiness']['child_age_message']['runtime_migration_allowed']) && str_contains((string)($contract['migration_readiness']['child_age_message']['blocker'] ?? ''), 'integer array') && count((array)($contract['migration_readiness']['child_age_message']['required_before_runtime'] ?? [])) === 2);
