...
$application = (string)file_get_contents($root . '/services/NeedApplicationService.php');
$ageParser = (string)file_get_contents($root . '/services/ChildAgesParser.php');
$valueContract = (string)file_get_contents($root . '/services/ChildAgeValueContract.php');
$failed = 0;
...
childrenAgesContractCheck('resolver-to-storage projection is wired through application boundary', !empty($contract['child_ages']['target_projection_required']) && ($contract['child_ages']['resolver_output_representation'] ?? null) === 'integer_array' && ($contract['child_ages']['application_input_representation'] ?? null) === 'comma_space_joined_decimal_string' && ($contract['child_ages']['projection_contract'] ?? null) === 'ChildAgeValueContract::toStorage' && strpos($valueContract, 'public static function toStorage') !== false && strpos($application, 'ChildAgeValueContract::toStorage($storageValue, $childrenCount)') !== false);
childrenAgesContractCheck('canonical child-age parser is wired and legacy parser is not a handler owner', ($contract['child_ages']['parser_parity_contract'] ?? null) === 'ChildAgesParser::parse' && strpos($ageParser, 'class ChildAgesParser') !== false && strpos($handler, 'ChildAgeValueContract::parseLegacyInput') === false);
childrenAgesContractCheck('child callback migration preserves zero-children age value', !empty($contract['migration_readiness']['child_callback']['runtime_migration_allowed']) && !empty($contract['migration_readiness']['child_callback']['preserve_existing_child_age_value_when_zero']) && !empty($contract['migration_readiness']['child_callback']['fail_closed_on_missing_step']));
childrenAgesContractCheck('child-age runtime keeps bounded authorization and canonical application completion', !empty($contract['migration_readiness']['child_age_message']['runtime_migration_allowed']) && ($contract['migration_readiness']['child_age_message']['authorized_scope'] ?? null) === 'StateMessageHandler statusAge free-text branch only' && ($contract['migration_readiness']['child_age_message']['contract_production_green_pr'] ?? null) === 687 && !empty($contract['migration_readiness']['child_age_message']['canonical_resolver_application_completed']) && array_key_exists('blocker', $contract['migration_readiness']['child_age_message']) && $contract['migration_readiness']['child_age_message']['blocker'] === null && count((array)($contract['migration_readiness']['child_age_message']['completed_before_runtime'] ?? [])) === 4 && count((array)($contract['migration_readiness']['child_age_message']['required_before_runtime'] ?? [])) === 0);
...
childrenAgesContractCheck('wizard child ages use canonical resolver/application path with child-count context', strpos($handler, "'child_ages',") !== false && strpos($handler, "['children'=>\$childCount]") !== false && strpos($handler, 'NeedApplicationService::resolveAndApplyExistingWizardStep(') !== false);
...
childrenAgesContractCheck('child callback and free-text ages both check application', !empty($contract['children']['current_application_result_is_checked']) && !empty($contract['child_ages']['current_application_result_is_checked']) && !empty($contract['migration_readiness']['child_age_message']['runtime_migration_completed']));
