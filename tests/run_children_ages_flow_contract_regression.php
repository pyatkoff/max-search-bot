...
childrenAgesContractCheck('resolver-to-storage projection is executable but not wired', empty($contract['child_ages']['target_projection_required']) && ($contract['child_ages']['resolver_output_representation'] ?? null) === 'integer_array' && ($contract['child_ages']['application_input_representation'] ?? null) === 'comma_space_joined_decimal_string' && ($contract['child_ages']['projection_contract'] ?? null) === 'ChildAgeValueContract::toStorage' && strpos($valueContract, 'public static function toStorage') !== false);
childrenAgesContractCheck('legacy parser parity contract is executable and wired once', ($contract['child_ages']['parser_parity_contract'] ?? null) === 'ChildAgeValueContract::parseLegacyInput' && strpos($valueContract, 'public static function parseLegacyInput') !== false && substr_count($handler, 'ChildAgeValueContract::parseLegacyInput') === 1);
...
childrenAgesContractCheck('child-age runtime is authorized only after production-green contract', !empty($contract['migration_readiness']['child_age_message']['runtime_migration_allowed']) && ($contract['migration_readiness']['child_age_message']['authorized_scope'] ?? null) === 'StateMessageHandler statusAge free-text branch only' && ($contract['migration_readiness']['child_age_message']['contract_production_green_pr'] ?? null) === 687 && array_key_exists('blocker', $contract['migration_readiness']['child_age_message']) && $contract['migration_readiness']['child_age_message']['blocker'] === null && count((array)($contract['migration_readiness']['child_age_message']['completed_before_runtime'] ?? [])) === 4 && count((array)($contract['migration_readiness']['child_age_message']['required_before_runtime'] ?? [])) === 0);
