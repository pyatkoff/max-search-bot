...
ppCheck('inbox explains when all-open urgency is more urgent than the projected pinned task',strpos($inboxJs,'function hiddenTaskUrgency(c)')!==false&&strpos($inboxJs,'operational_task_due_state')!==false&&strpos($inboxJs,'Ещё есть просроченная')!==false&&strpos($inboxJs,'Ещё есть на сегодня')!==false);
...
ppCheck('kanban explains hidden urgent work without changing the projected control target',strpos($kanbanTasksJs,'WorkspaceV2Inbox?.hiddenTaskUrgency')!==false&&strpos($kanbanTasksJs,'Есть ещё просроченная задача')!==false&&strpos($kanbanTasksJs,'Есть ещё задача на сегодня')!==false&&strpos($kanbanTasksJs,'c.next_task_id')!==false&&strpos($kanbanCss,'.kanbanTaskHiddenUrgency.overdue')!==false);
ppCheck('kanban shows when more open tasks exist behind the projected task',strpos($kanbanTasksJs,'function extraTaskCountLabel(count)')!==false&&strpos($kanbanTasksJs,'c.open_task_count')!==false&&strpos($kanbanTasksJs,"?'открытая'")!==false&&strpos($kanbanTasksJs,"?'открытые':'открытых'")!==false&&strpos($kanbanTasksJs,'Ещё ${extraTaskCountLabel(count)}')!==false&&strpos($kanbanTasksJs,'${extraTaskSignal(c)}')!==false);
