...
mdpCheck('queue counts materialize waiting and mine only once each',substr_count($queues,'ManagerConversationService::list(')===2 && strpos($queues,"ManagerConversationService::list(\$managerId,'waiting',200,\$projectKey)")!==false && strpos($queues,"ManagerConversationService::list(\$managerId,'mine',200,\$projectKey)")!==false);
...
mdpCheck('notification unread preserves raw waiting plus mine contract',strpos($queues,'foreach(array_merge($rawWaiting,$mine) as $row)')!==false);
