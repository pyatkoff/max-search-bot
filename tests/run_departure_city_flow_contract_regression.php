...
departureCityContractCheck('value contract does not authorize runtime migration', empty($contract['migration_readiness']['runtime_migration_allowed']) && ($contract['migration_readiness']['reason'] ?? null) === 'value_contract_only' && count((array)($contract['migration_readiness']['required_before_runtime'] ?? [])) === 7);
departureCityContractCheck('value contract is executable but runtime-disconnected', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DepartureCityValueContract.php' && empty($contract['executable_value_contract']['runtime_connected']) && ($contract['executable_value_contract']['no_flight_id'] ?? null) === 99);
