...
departureCityContractCheck('callback-only runtime does not authorize another caller', empty($contract['migration_readiness']['runtime_migration_allowed']) && ($contract['migration_readiness']['reason'] ?? null) === 'callback_only_runtime_free_text_blocked' && count((array)($contract['migration_readiness']['required_before_runtime'] ?? [])) === 7);
departureCityContractCheck('value contract is connected only to callback runtime', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DepartureCityValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === 'callback_only' && ($contract['executable_value_contract']['no_flight_id'] ?? null) === 99);
