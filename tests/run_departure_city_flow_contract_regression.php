...
departureCityContractCheck('callback application is update-only and fail-closed', ($contract['callback']['application'] ?? null) === 'ExistingWizardStepApplicationService::apply' && !empty($contract['callback']['application_result_is_checked']) && ($contract['callback']['missing_or_pre_start_step_current_behavior'] ?? null) === 'consumed_fail_closed_without_mutation_or_progression');
departureCityContractCheck('free-text application is update-only and fail-closed', ($contract['free_text']['application'] ?? null) === 'ExistingWizardStepApplicationService::apply' && !empty($contract['free_text']['application_result_is_checked']) && ($contract['free_text']['missing_or_pre_start_step_current_behavior'] ?? null) === 'fail_closed_without_mutation_or_progression');
departureCityContractCheck('completed runtime callers do not authorize another caller', empty($contract['migration_readiness']['runtime_migration_allowed']) && ($contract['migration_readiness']['reason'] ?? null) === 'callback_and_free_text_runtime_complete_next_caller_blocked' && count((array)($contract['migration_readiness']['required_before_runtime'] ?? [])) === 7);
departureCityContractCheck('value contract is connected to callback and free-text runtime only', ($contract['executable_value_contract']['owner'] ?? null) === 'services/DepartureCityValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === 'callback_and_free_text' && ($contract['executable_value_contract']['no_flight_id'] ?? null) === 99);

foreach ($payloadMap as $payload => $value) {
    departureCityContractCheck("view exposes {$payload}", strpos($view, "'{$payload}'") !== false);
}
departureCityContractCheck('callback normalizes and applies through update-only boundary', strpos($callback, 'DepartureCityValueContract::fromCallbackPayload($q)') !== false && strpos($callback, 'ExistingWizardStepApplicationService::apply(') !== false && strpos($callback, 'MaxSearchApi::$statusCityChoose,') !== false);
departureCityContractCheck('callback checks city normalization and write results', strpos($callback, 'if ($city === null) return true;') !== false && strpos($callback, 'if (!ExistingWizardStepApplicationService::apply(') !== false && strpos($callback, 'MaxSearchApi::saveLastValue($chatId, MaxSearchApi::$statusCityChoose') === false);
...
departureCityContractCheck('free text normalizes and applies through update-only boundary', strpos($handler, 'DepartureCityValueContract::fromDirectoryId($cityRes["ID"] ?? null)') !== false && strpos($handler, 'ExistingWizardStepApplicationService::apply(') !== false && strpos($handler, 'MaxSearchApi::saveLastValue($chat_id,MaxSearchApi::$statusCityChoose') === false);
departureCityContractCheck('free text checks normalization and application before advancing', strpos($handler, 'if($cityId === null) return;') !== false && strpos($handler, 'if(!ExistingWizardStepApplicationService::apply(') !== false && strpos($handler, 'MaxSearchApi::showCountryButtons($chat_id)') !== false);
