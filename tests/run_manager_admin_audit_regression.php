...
$editSafety=(string)file_get_contents($root.'/manager/assets/admin-edit-safety.js');
...
maaCheck('admin edit safety companion is cache-busted and loaded after canonical admin logic',strpos($admin,'admin-edit-safety.js?v=')!==false&&strpos($admin,'assets/admin.js?v=')<strpos($admin,'admin-edit-safety.js?v='));
maaCheck('project manager and rule edit modes are explicit and accessible',strpos($editSafety,"ensureModeStatus(projectCard,'projectEditMode')")!==false&&strpos($editSafety,"ensureModeStatus(managerCard,'managerEditMode')")!==false&&strpos($editSafety,"ensureModeStatus(ruleCard,'ruleEditMode')")!==false&&strpos($editSafety,"el.setAttribute('role','status')")!==false&&strpos($editSafety,"el.setAttribute('aria-live','polite')")!==false&&strpos($editSafety,"classList.toggle('editing',id>0)")!==false);
maaCheck('editing an existing admin record exposes a dedicated cancel action',strpos($editSafety,"cancelProjectEdit")!==false&&strpos($editSafety,"cancelManagerEdit")!==false&&strpos($editSafety,"cancelRuleEdit")!==false&&strpos($editSafety,"classList.toggle('hidden',id<=0)")!==false);
maaCheck('cancel project edit restores safe new-project defaults without server mutation',strpos($editSafety,"$('projectId').value=''")!==false&&strpos($editSafety,"$('projectKey').value=''")!==false&&strpos($editSafety,"$('projectName').value=''")!==false&&strpos($editSafety,"$('projectActive').checked=true")!==false&&strpos($editSafety,"api(")===false);
maaCheck('cancel manager edit restores safe new-manager defaults without changing shifts or routing',strpos($editSafety,"$('managerId').value=''")!==false&&strpos($editSafety,"$('managerRole').value='manager'")!==false&&strpos($editSafety,"$('managerPriority').value=0")!==false&&strpos($editSafety,"$('managerActive').checked=true")!==false&&strpos($editSafety,"document.querySelectorAll('#managerProjects input').forEach(x=>x.checked=false)")!==false&&strpos($editSafety,"api(")===false);
maaCheck('cancel rule edit restores safe new-rule defaults without routing mutation',strpos($editSafety,"$('ruleId').value=''")!==false&&strpos($editSafety,"$('ruleValue').value=''")!==false&&strpos($editSafety,"$('ruleBonus').value=0")!==false&&strpos($editSafety,"$('ruleComment').value=''")!==false&&strpos($editSafety,"$('ruleActive').checked=true")!==false&&strpos($editSafety,'manager.selectedIndex=0')!==false&&strpos($editSafety,'type.selectedIndex=0')!==false&&strpos($editSafety,"api(")===false);
maaCheck('admin edit controls stay synchronized after canonical successful save reset',strpos($editSafety,"new MutationObserver(syncProjectMode).observe(projectStatus")!==false&&strpos($editSafety,"new MutationObserver(syncManagerMode).observe(managerStatus")!==false&&strpos($editSafety,"new MutationObserver(syncRuleMode).observe(ruleStatus")!==false);
maaCheck('admin edit safety controls have distinct editing and secondary-action presentation',strpos($css,'.adminEditMode.editing')!==false&&strpos($css,'.btnSecondary')!==false&&strpos($css,'.adminFormActions')!==false);
