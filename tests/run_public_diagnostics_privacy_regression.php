...
publicDiagCheck('production publisher commits only public-safe aggregate artifacts',strpos($publish,'cp production_snapshot.json live_session_report.json')===false&&strpos($publish,'git add -A architecture_inventory.json ops_status.json autopilot_snapshot.json')!==false&&strpos($publish,"git rm -f --ignore-unmatch production_snapshot.json")!==false);
publicDiagCheck('hourly live publisher removes per-session artifacts',strpos($live,'public_live_status.json')!==false&&strpos($live,"git rm -f --ignore-unmatch live_sessions.json daily_sessions.json live_anomalies.json")!==false&&strpos($live,'git add live_sessions.json daily_sessions.json live_anomalies.json')===false);
publicDiagCheck('deploy telemetry does not republish downloaded raw json',strpos($deploy,"find ../production-diagnostics -maxdepth 1 -type f -name '*.json' -exec cp")===false&&strpos($deploy,'git add deploy_status.json *.json')===false&&strpos($deploy,"git rm -f --ignore-unmatch 'diag-*.json'")!==false);
