...
$pushEndpoint=(string)file_get_contents($root.'/manager/push.php');
...
nCheck('push subscription write carries the current Manager csrf token',strpos($pushEndpoint,"if(\$action==='subscribe')")!==false&&strpos($pushEndpoint,'ManagerHttp::requireCsrf($data);')!==false&&strpos($js,"function csrf(){return String(window.WorkspaceV2?.S?.csrf||'');}")!==false&&strpos($js,"action:'subscribe',csrf:csrf(),subscription:sub.toJSON()")!==false);
...
nCheck('already-granted push can self-recover without permission prompt',strpos($js,'function maybeRecoverGranted(root)')!==false&&strpos($js,"Notification.permission!=='granted'")!==false&&strpos($js,"requestPermission:false,showConfirmation:false,interactive:false")!==false&&strpos($js,'maybeRecoverGranted(root);')!==false);
nCheck('self-recovery is limited to missing or unhealthy subscriptions and attempted once per page',strpos($js,"['no_subscription','subscription_unhealthy'].includes")!==false&&strpos($js,'autoRecoveryAttempted||enableInFlight')!==false&&strpos($js,'autoRecoveryAttempted=true')!==false);
nCheck('background recovery does not loop refresh or show confirmation notification',strpos($js,'if(showConfirmation){try{await reg.showNotification')!==false&&strpos($js,'if(interactive)setTimeout(()=>refresh(),500)')!==false&&strpos($js,'showConfirmation:false,interactive:false')!==false);
...
nCheck('push setup transient and failure states reuse preserved shift state',strpos($js,"render(root,withCurrentWorking({notification_path_usable:false,notification_path_reason:'no_subscription'}))")!==false&&strpos($js,"finalStatus=withCurrentWorking({notification_path_usable:false,notification_path_reason:'permission_denied'})")!==false&&strpos($js,"render(root,withCurrentWorking({notification_path_usable:false,notification_path_reason:'health_check_failed'}))")!==false);
nCheck('push setup renders canonical backend health instead of inventing success state',strpos($js,'!saved.push_status')!==false&&strpos($js,'lastStatus=saved.push_status;finalStatus=lastStatus')!==false&&strpos($js,"finalStatus=withCurrentWorking({notification_path_usable:true,notification_path_reason:'healthy_subscription',healthy_subscription_count:1})")===false);
nCheck('push subscribe endpoint returns canonical manager health after persistence',strpos($pushEndpoint,"require_once \$baseDir.'/services/ConversationDb.php'")!==false&&strpos($pushEndpoint,"require_once \$baseDir.'/services/ManagerPushHealth.php'")!==false&&strpos($pushEndpoint,'ManagerPushHealth::statusForManager(ConversationDb::connection(),$managerId)')!==false&&strpos($pushEndpoint,"['ok'=>true,'push_status'=>\$pushStatus]")!==false);
