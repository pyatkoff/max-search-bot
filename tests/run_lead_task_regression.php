...
ltCheck('task detail exposes canonical urgency without client-side date rules',strpos($taskService,"\$row['due_state']=self::dueState")!==false&&strpos($taskService,' AS overdue')!==false&&strpos($taskJs,'task.due_state')!==false&&strpos($taskJs,"new Date(localValue)")!==false&&strpos($taskJs,'Europe/Kaliningrad')===false);
