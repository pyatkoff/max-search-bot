...
ltCheck('task detail is read-only visible data',strpos($api,"pipelineDetailPart('tasks',\$id,static fn()=>LeadTaskService::listForConversation(\$id)")!==false&&strpos($api,"'tasks'=>\$tasks")!==false);
...
ltCheck('lead card delegates task rendering while task module owns mutations',strpos($leadCardJs,'WorkspaceV2Tasks.render(root,{tasks,canEdit,conversationId})')!==false&&strpos($leadCardJs,"pipe('create_task'")===false&&strpos($taskJs,"W.pipe('create_task'")!==false&&strpos($taskJs,"W.pipe('update_task'")!==false&&strpos($taskJs,"W.pipe('set_task_completed'")!==false&&strpos($taskJs,"W.pipe('set_task_pinned'")!==false);
...
ltCheck('task module returns task create success to task UI',strpos($taskJs,'async function createTask')!==false&&strpos($taskJs,"W.pipe('create_task'")!==false&&strpos($taskJs,'if(!j.ok)return false')!==false&&strpos($taskJs,'return refreshLead(target)')!==false);
...
ltCheck('repeated completion mutations are idempotent and preserve the original completion transition',strpos($taskService,'status<>?')!==false&&strpos($taskService,'SELECT status FROM lead_tasks')!==false&&strpos($taskService,'(string)$current===$status')!==false);
...
ltCheck('V2 inbox task filters use the most-urgent open-task due state',strpos($inboxService,"\$operationalDueState=(string)(\$row['operational_task_due_state']??\$dueState)")!==false&&strpos($inboxService,"\$taskFilter==='planned'&&(!\$hasTask||\$operationalDueState!=='upcoming')")!==false);
