...
ltCheck('open lead tasks can be edited without changing technical state',strpos($taskService,'function update(')!==false&&strpos($taskService,"status='open'")!==false&&strpos($taskService,'SET title=?,due_at_utc=?')!==false&&strpos($taskService,'UPDATE conversations SET status')===false);
