...
ltCheck('task API reuses conversation visibility and shared pipeline edit guard',strpos($api,'pipelineConversation($id')!==false&&strpos($api,"\$action==='create_task'")!==false&&strpos($api,"\$action==='set_task_completed'")!==false&&strpos($api,"\$action==='set_task_pinned'")!==false&&substr_count($api,'ManagerHttp::requireConversationEdit($c,$m);')>=6&&strpos($http,'ManagerRequestContext::canEditAssignedConversation')!==false&&strpos($http,"'error'=>'forbidden'")!==false);
