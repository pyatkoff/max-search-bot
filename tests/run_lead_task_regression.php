...
ltCheck('lead card delegates task rendering and mutations to task module',strpos($leadCardJs,'WorkspaceV2Tasks.render')!==false&&strpos($leadCardJs,"pipe('create_task'")!==false&&strpos($leadCardJs,"pipe('set_task_completed'")!==false&&strpos($leadCardJs,"pipe('set_task_pinned'")!==false&&strpos($leadCardJs,'onPin:pinTask')!==false);
...
ltCheck('LeadTaskService owns pinned task persistence and priority ordering',strpos($taskService,'function setPinned')!==false&&strpos($taskService,"status='open' AND is_pinned=?")!==false&&strpos($taskService,"t.status='open' AND t.is_pinned=1")!==false&&strpos($taskService,"'open',0")!==false);
...
ltCheck('task pinning stays role-gated and outside technical status',strpos($api,"\$action==='set_task_pinned'")!==false&&strpos($api,'LeadTaskService::setPinned')!==false&&strpos($api,'UPDATE conversations SET status')===false);
...
ltCheck('task signal stays read-only in inbox',strpos($inboxJs,"pipe('create_task'")===false&&strpos($inboxJs,"pipe('set_task_completed'")===false&&strpos($inboxJs,"pipe('set_task_pinned'")===false);
