...
inboxCheck('manager-request queue is canonical server-rendered workspace markup',strpos($page,'<button data-q="requested">Запросили менеджера</button>')!==false&&strpos($bootstrap,'ensureManagerRequestQueue')===false&&strpos($bootstrap,"createElement('button')")===false&&strpos($bootstrap,'window.WorkspaceV2?.boot()')!==false);
