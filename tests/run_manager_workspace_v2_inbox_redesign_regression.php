...
inboxCheck('pipeline API gates manager filtering to admin role',strpos($pipelineApi,'$managerFilter=$isAdmin?')!==false&&strpos($pipelineApi,"(string)(\$data['manager_filter']??'')")!==false);
inboxCheck('pipeline API applies source id without changing routing',strpos($pipelineApi,"$sourceId=max(0,(int)(\$data['source_id']??0))")!==false&&strpos($pipelineApi,"(int)(\$r['source_id']??0)===\$sourceId")!==false);
...
inboxCheck('redesign does not mutate shifts metrika or routing bonuses',stripos($js.$filters.$pipeline.$pipelineApi.$filterService.$css,'set_working')===false&&stripos($js.$filters.$pipeline.$filterService.$css,'metrika')===false&&stripos($js.$filters.$pipeline.$filterService.$css,'yclid')===false&&stripos($filterService,'bonus')===false);
