...
mobileCheck('reply draft cleanup is pinned to successful source conversation owner and unchanged text',strpos($conversation,'drafts.delete(target)')!==false&&strpos($conversation,'const stillCurrent=openSeq===generation&&Number(S.current)===target')!==false&&strpos($conversation,'owner===replySessionOwner&&owner===Number(S.manager?.id)&&drafts.get(target)?.text===draftText')!==false&&strpos($conversation,"if(stillCurrent){\n      if($('replyText').value===draftText)$('replyText').value='';")!==false&&strpos($conversation,'drafts.delete(Number(S.current))')===false);
mobileCheck('typing and quick replies update the current draft',strpos($conversation,"reply.addEventListener('input',()=>{saveDraft();autoGrow()})")!==false&&strpos($conversation,'b.onclick=()=>addQuickReply(b.dataset.reply)')!==false&&strpos($conversation,'saveDraft();autoGrow();reply.focus()')!==false);
