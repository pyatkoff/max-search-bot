...
ProjectConfig::resetForTests(['id'=>'test','search'=>['base_domain'=>'https://example.test','tracking_base_domain'=>'https://tracking.example.test','claim_path'=>'/search/{code}/'],'state'=>['v2_store_dir'=>'runtime/test_trip_state']]);
v2check('project id',ProjectConfig::projectId(),'test');
v2check('claim url',ProjectConfig::claimUrl('abc','123'),'https://example.test/search/abc/?yclid=123');
v2check('tracking base fallback',ProjectConfig::trackingBaseDomain(),'https://tracking.example.test');
...
v2check('claim url uses public override',ProjectConfig::claimUrl('abc','123'),'https://public.override.test/search/abc/?yclid=123');
