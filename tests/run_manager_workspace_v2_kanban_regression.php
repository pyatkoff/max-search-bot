...
kbCheck('quick task reuses canonical backend task owner and authorization',strpos($api,"if(\$action==='create_task'")!==false&&strpos($api,'LeadTaskService::create')!==false&&strpos($api,"if(!\$can)pipelineOut(['ok'=>false,'error'=>'forbidden'],403)")!==false);
...
kbCheck('quick task has explicit save error and duplicate-submit guard',strpos($kanban,"save.disabled=true")!==false&&strpos($kanban,"error.textContent='Не удалось сохранить задачу'")!==false&&strpos($kanban,"if(!title)")!==false);
