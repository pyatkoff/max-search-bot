...
kbCheck('kanban overdue column summary uses all-task operational urgency',strpos($kanban,"if(operationalTaskState(c)==='overdue')overdue+=1")!==false);
kbCheck('kanban operational urgency retains backward-compatible projected fallback',strpos($kanban,"if(Number(c.next_task_overdue||0)===1||c.next_task_overdue===true)return'overdue'")!==false&&strpos($kanban,"String(c.next_task_due_state||'')==='today'?'today':'soon'")!==false);
...
kbCheck('column headers retain actionable task workload and sales summaries',strpos($kanban,'function columnSummary')!==false&&strpos($kanban,'awaiting_first_reply')!==false&&strpos($kanban,"String(c.next_task_title||'').trim()!==''")!==false&&strpos($kanban,'`${tasks} задач`')!==false&&strpos($kanban,'unread_count')!==false&&strpos($kanban,'function columnSalesTotal')!==false&&strpos($kanban,'lead_sale_amount')!==false&&strpos($kanban,'Продажи ${sales}')!==false);
...
kbCheck('kanban refresh preserves horizontal board position',strpos($kanban,'function captureScroll(root)')!==false&&strpos($kanban,'left:Number(root.scrollLeft||0)')!==false&&strpos($kanban,'root.scrollLeft=Number(state.left||0)')!==false&&strpos($kanban,'scrollState=captureScroll(root)')!==false&&strpos($kanban,'restoreScroll(root,scrollState)')!==false);
kbCheck('kanban refresh preserves per-stage vertical card position',strpos($kanban,'data-stage-key=')!==false&&strpos($kanban,"querySelectorAll('.kanbanColumn[data-stage-key]')")!==false&&strpos($kanban,'cards.scrollTop=Number(state.columns[key]||0)')!==false&&strpos($kanban,'data-stage-key="__other"')!==false);
