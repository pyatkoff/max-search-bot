...
kbCheck('quick task recovers after rejected request without clearing draft',strpos($kanban,"try{j=await pipe('create_task'")!==false&&strpos($kanban,"catch(e){form.dataset.saving='0';save.disabled=false;error.textContent='Не удалось сохранить задачу';return}")!==false&&strpos($kanban,"title=form.querySelector('.kanbanTaskTitle')?.value.trim()")!==false&&strpos($kanban,"form.querySelector('.kanbanTaskTitle').value=''")===false);
