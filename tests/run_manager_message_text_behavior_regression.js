...
assert.equal(preview('Нет последнего сообщения','customer',''),'Нет последнего сообщения','empty display text preserves the empty-message placeholder');
const callback='g1_1234abcd_show_tours';
const callbackLabel='Нажата кнопка «Показать туры»';
assert.equal(preview(callback,'customer',callbackLabel),callbackLabel,'Recorded button event uses its read-only display projection');
assert.equal(preview(callback,'customer'),callback,'The same text typed by a customer stays literal without callback metadata');
assert.equal(preview('🔴 Клиент недоступен в MAX · '+callback,'customer','🔴 Клиент недоступен в MAX · '+callbackLabel),'🔴 Клиент недоступен в MAX · '+callbackLabel,'Delivery warning survives callback projection');
