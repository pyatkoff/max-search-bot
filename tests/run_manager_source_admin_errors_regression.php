...
sourceCheck('source fallback mode fails closed instead of silently disabling fallback',strpos($service,"if(!in_array(\$fallbackMode,['none','delayed','immediate'],true))return['ok'=>false,'error'=>'invalid_fallback_mode'];")!==false&&strpos($service,"$fallbackMode=in_array($fallbackMode,['none','delayed','immediate'],true)?$fallbackMode:'none'")===false);
...
sourceCheck('routing UI maps backend source failures to specific messages',strpos($routingJs,"duplicate_source_key:'Источник с таким кодом уже существует")!==false&&strpos($routingJs,"invalid_primary_group:'Основная группа недоступна")!==false&&strpos($routingJs,"source_project_mismatch:'Источник относится к другому проекту. Обновите список и выберите источник заново.'")!==false&&strpos($routingJs,"save_failed:'Источник не сохранён из-за ошибки сервера")!==false);
sourceCheck('routing UI explains invalid fallback mode and focuses the stale control',strpos($routingJs,"invalid_fallback_mode:'Выбран неподдерживаемый режим fallback")!==false&&strpos($routingJs,"else if(j.error==='invalid_fallback_mode')$('fallbackMode').focus()")!==false);
