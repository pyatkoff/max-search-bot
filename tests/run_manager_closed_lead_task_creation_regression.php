...
closedTaskCheck('task API derives current outcome from canonical pipeline owner',strpos($api,'function pipelineOutcome(int $id):string')!==false&&strpos($api,'SalesPipelineService::outcomeForConversation($id)')!==false);
closedTaskCheck('task API rejects new task creation after lead close',strpos($api,"if(pipelineOutcome(\$id)!=='open')ManagerHttp::respond(['ok'=>false,'error'=>'lead_closed'],409)")!==false);
closedTaskCheck('existing task cleanup remains allowed after lead close',strpos($api,"if(\$action==='update_task')")!==false&&strpos($api,"if(\$action==='set_task_completed')")!==false&&strpos($api,"if(\$action==='set_task_pinned')")!==false&&substr_count($api,"pipelineOutcome(\$id)!=='open'")===1);
