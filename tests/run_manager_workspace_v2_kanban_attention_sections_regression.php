...
checkAttention('closed outcomes are excluded from actionable overdue and today column counts',strpos($kanban,"const outcome=String(c.lead_outcome||'open')")!==false&&strpos($kanban,"if(outcome==='open'){const taskState=operationalTaskState(c)")!==false&&strpos($kanban,"else if(String(c.lead_outcome||'open')==='open')missing++")!==false);
