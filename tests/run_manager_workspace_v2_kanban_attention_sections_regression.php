...
checkAttention('column summary shows won deal count alongside existing sale amount',strpos($kanban,'unread=0,won=0')!==false&&strpos($kanban,"if(outcome==='won')won+=1")!==false&&strpos($kanban,'if(won)parts.push(`${won} продаж`)')!==false&&strpos($kanban,'columnSalesTotal(rows)')!==false);
