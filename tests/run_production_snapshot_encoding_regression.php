...
pseCheck('schema probe reads information schema columns independent of metadata key casing', strpos($source, '$q->fetchAll(PDO::FETCH_COLUMN)') !== false, true);
pseCheck('production snapshot traces latest Telegram manager callback without publishing chat identity', strpos($source, 'function recentTelegramManagerCallback(PDO $pdo,string $structuredPath):array') !== false && strpos($source, "m.channel='telegram'") !== false && strpos($source, "m.text LIKE '%manager_request%'") !== false && strpos($source, "'recent_telegram_manager_callback'=>[]") !== false && strpos($source, "'external_chat_id'=>") === false, true);
pseCheck('Telegram manager trace correlates bounded guard transport webhook and database outcomes', strpos($source, "'ai_outbound_after'") !== false && strpos($source, "'waiting_events_after'") !== false && strpos($source, "recentStructuredComponentEvents(\$structuredPath,'interaction_guard',100)") !== false && strpos($source, "recentStructuredComponentEvents(\$structuredPath,'telegram_transport',100)") !== false && strpos($source, "recentStructuredComponentEvents(\$structuredPath,'incoming_dispatch',100)") !== false && strpos($source, "recentStructuredComponentEvents(\$structuredPath,'telegram_webhook',100)") !== false && strpos($source, "array_slice(\$events,-20)") !== false, true);
