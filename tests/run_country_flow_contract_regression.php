...
countryContractCheck('executable value contract is present and callback-only', ($contract['executable_value_contract']['owner'] ?? null) === 'services/CountryValueContract.php' && ($contract['executable_value_contract']['runtime_connected'] ?? null) === 'callback_only' && ($contract['executable_value_contract']['directory_id_projection'] ?? null) === 'positive_canonical_integer_or_decimal_string_to_exact_decimal_storage_string' && ($contract['executable_value_contract']['callback_payload_parser'] ?? null) === 'pick_country_<positive_canonical_id>' && ($contract['executable_value_contract']['covered_popular_ids'] ?? null) === [1, 2, 4, 8, 9, 12] && ($contract['executable_value_contract']['covered_additional_id'] ?? null) === 347);
...
countryContractCheck('free text currently stores PDO id with unchecked direct write', strpos($handler, 'MaxSearchApi::saveLastValue($chat_id,MaxSearchApi::$statusContryChoose,$countryRes["ID"])') !== false && empty($contract['free_text']['application_result_is_checked']) && ($contract['free_text']['missing_or_pre_start_step_current_behavior'] ?? null) === 'progresses_without_confirming_write');
...
countryContractCheck('free-text runtime migration remains explicitly blocked', empty($contract['migration_readiness']['runtime_migration_allowed']) && ($contract['migration_readiness']['reason'] ?? null) === 'callback_slice_requires_production_green_and_separate_free_text_authorization' && count((array)($contract['migration_readiness']['required_before_runtime'] ?? [])) === 6);
