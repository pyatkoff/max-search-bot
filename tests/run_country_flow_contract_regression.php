...
countryContractCheck('callback currently strips prefix and leaves write unchecked', strpos($callback, "MaxSearchApi::saveLastValue(\$chatId, MaxSearchApi::\$statusContryChoose, str_replace('pick_country_', '', \$q))") !== false && strpos($callback, "if (EditFlowService::finishIfNeeded(\$chatId, 'country'))") !== false && ($contract['callback']['payload_projection'] ?? null) === 'str_replace_prefix_without_value_validation' && empty($contract['callback']['application_result_is_checked']));
