...
countryContractCheck('resort hints stay independent from optional hotel refinements', strpos($hints, 'public static function seedCountry') !== false && strpos($defaults, 'DestinationHintService::seedCountry') !== false && strpos($defaults, "\$p['meal'] = 'all_inclusive';") === false && strpos($defaults, "\$p['stars'] = 4;") === false);
$countryDefaults = (array)($contract['ai']['turkey_egypt_defaults'] ?? []);
countryContractCheck('Turkey/Egypt hotel refinements stay unknown without tourist evidence', array_key_exists('meal_when_missing', $countryDefaults) && $countryDefaults['meal_when_missing'] === null && array_key_exists('stars_when_missing', $countryDefaults) && $countryDefaults['stars_when_missing'] === null);
