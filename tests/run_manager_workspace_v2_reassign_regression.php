...
ownerCheck('reassignment rejects inactive unknown or inaccessible target',strpos($service,'ManagerAuthService::byId($targetManagerId)')!==false&&strpos($service,'ManagerConversationAccessPolicy::canView($targetManagerId,$row)')!==false&&strpos($service,"(string)\$row['status']==='closed'")!==false);
...
ownerCheck('reassignment transfers only open tasks to the new owner in the same transaction',strpos($service,"UPDATE lead_tasks SET assigned_manager_id=?,updated_at=UTC_TIMESTAMP() WHERE conversation_id=? AND status='open'")!==false&&strpos($service,"execute([\$targetManagerId,\$conversationId])")!==false);
ownerCheck('reassignment preserves completed task ownership history',strpos($service,"WHERE conversation_id=? AND status='open'")!==false&&strpos($service,"status='done'")===false);
