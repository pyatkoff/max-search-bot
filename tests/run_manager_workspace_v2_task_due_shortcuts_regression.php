...
dsCheck('snooze respects dirty task editor guard and shared mutation recovery',strpos($tasks,"root.querySelectorAll('[data-task-snooze]')")!==false&&strpos($tasks,'if(blockForDirtyDraft(root))return')!==false&&strpos($tasks,'runMutation(el,row,()=>onSnooze(task,preset))')!==false);
