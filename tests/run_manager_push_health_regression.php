...
mphCheck('push health exposes public-safe aggregate reason counts',strpos($service,"'working_manager_count'")!==false && strpos($service,"'usable_notification_path_count'")!==false && strpos($service,"'no_subscription_count'")!==false && strpos($service,"'unhealthy_subscription_count'")!==false && strpos($service,"'other_unusable_count'")!==false);
...
mphCheck('genuinely renewed subscription clears stale delivery failure health without healing unchanged keys',strpos($push,'last_error_at=IF(p256dh<>VALUES(p256dh) OR auth_secret<>VALUES(auth_secret),NULL,last_error_at)')!==false && strpos($push,'last_error=IF(p256dh<>VALUES(p256dh) OR auth_secret<>VALUES(auth_secret),NULL,last_error)')!==false);
