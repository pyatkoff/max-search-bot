...
mphCheck('Workspace V2 distinguishes working without push',strpos($panel,"const working=!!status?.is_working")!==false && strpos($panel,"const usable=!!status?.notification_path_usable")!==false && strpos($panel,"root.className='notificationHealth '+(usable?'ok':'warn')")!==false);
...
mphCheck('Workspace V2 only claims notifications enabled for healthy server path',strpos($panel,"healthy_subscription:'Уведомления включены'")!==false && strpos($panel,"const usable=!!status?.notification_path_usable")!==false && strpos($panel,"label=reasonText(reason)")!==false);
