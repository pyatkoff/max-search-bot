...
$themeFixture=(string)@file_get_contents($root.'/tests/visual/workspace-v2-theme-fixture.html');
$theme=(string)@file_get_contents($root.'/manager/assets/workspace-v2-theme.css');
...
vqCheck('visual-system fixture exists',$themeFixture!=='');
vqCheck('visual-system stylesheet exists',$theme!=='');
...
vqCheck('captures visual-system evidence at all baseline widths',strpos($workflow,'theme-390-inbox.png')!==false&&strpos($workflow,'theme-430-lead.png')!==false&&strpos($workflow,'theme-768-conversation.png')!==false&&strpos($workflow,'theme-1440-desktop.png')!==false);
...
vqCheck('production workspace loads visual system after feature styles',strpos($workspace,'assets/workspace-v2-theme.css')!==false&&strrpos($workspace,'assets/workspace-v2-theme.css')>strrpos($workspace,'assets/workspace-v2-kanban.css'));
vqCheck('visual-system fixture uses production theme',strpos($themeFixture,'../../manager/assets/workspace-v2-theme.css')!==false&&strpos($themeFixture,'waitUrgent active')!==false&&strpos($themeFixture,'msg manager')!==false&&strpos($themeFixture,'leadZone')!==false);
vqCheck('visual system defines shared semantic tokens',strpos($theme,'--ws-surface:')!==false&&strpos($theme,'--ws-text:')!==false&&strpos($theme,'--ws-accent:')!==false&&strpos($theme,'--ws-warning:')!==false&&strpos($theme,'--ws-danger:')!==false&&strpos($theme,'--ws-focus:')!==false);
vqCheck('visual system provides keyboard focus visibility',strpos($theme,':focus-visible')!==false&&strpos($theme,'var(--ws-focus)')!==false);
vqCheck('visual system keeps mobile safe-area composer support',strpos($theme,'env(safe-area-inset-bottom)')!==false);
vqCheck('fixtures have no real production endpoints',strpos($fixture,'anytour.online')===false&&strpos($fixture,'api.php')===false&&strpos($fixture,'pipeline-api.php')===false&&strpos($kanbanFixture,'anytour.online')===false&&strpos($kanbanFixture,'api.php')===false&&strpos($kanbanFixture,'pipeline-api.php')===false&&strpos($themeFixture,'anytour.online')===false&&strpos($themeFixture,'api.php')===false&&strpos($themeFixture,'pipeline-api.php')===false);
