...
checkConversationUi('conversation detail loads against target before committing current id',strpos($js,"Promise.allSettled([api('detail',{conversation_id:target}),pipe('detail',{conversation_id:target})])")!==false&&strpos($js,'S.current=target;S.detail={...d,lead:p}')!==false);
checkConversationUi('failed core detail keeps previous conversation active and explicit',strpos($js,'if(!d?.ok)')!==false&&strpos($js,'if(!d?.ok||!p?.ok)')===false&&strpos($js,'Текущий диалог не изменён.')!==false&&strpos($js,'conversationLoadStatus')!==false&&strpos($css,'.conversationLoadStatus.error')!==false);
checkConversationUi('failed secondary lead detail cannot block transcript',strpos($js,"const d=detailResult.status==='fulfilled'?detailResult.value:null,p=leadResult.status==='fulfilled'&&leadResult.value?.ok?leadResult.value:null")!==false&&strpos($js,'if(p)window.WorkspaceV2LeadCard?.render(p,c);else window.WorkspaceV2LeadCard?.renderUnavailable(c)')!==false);
checkConversationUi('stale rapid-click detail response cannot overwrite newer selection',strpos($js,'openSeq=0')!==false&&strpos($js,'seq=++openSeq')!==false&&strpos($js,'if(seq!==openSeq)return false')!==false);
...
checkConversationUi('composer has quick replies without auto send',strpos($page,'class="quickReplies"')!==false&&strpos($js,'b.onclick=()=>addQuickReply(b.dataset.reply)')!==false);
