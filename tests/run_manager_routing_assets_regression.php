...
checkRouting('source admin persists and restores start mode',strpos($js,'handlingMode')!==false&&strpos($js,'handling_mode')!==false&&strpos($api,"handling_mode']??'ai'")!==false);
...
checkRouting('manager choice suppresses AI even when transport fails',strpos($sourceHandling,"'source_policy'")!==false&&strpos($sourceHandling,"'source_choice'")!==false&&substr_count($sourceHandling,'return true;')>=2);
checkRouting('handoff back restores self service',strpos($sourceHandling,"'handoff_back'")!==false&&strpos($sourceHandling,'self::AI')!==false);
