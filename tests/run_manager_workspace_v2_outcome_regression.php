...
outcomeCheck('outcome save is enabled only after a user edit',strpos($leadCardJs,'id="saveOutcome" class="actionBtn primary outcomeSave" type="button" disabled')!==false&&strpos($pipelineJs,'function setOutcomeDirty(dirty=true)')!==false&&strpos($pipelineJs,'button.disabled=!dirty')!==false&&strpos($pipelineJs,'isOutcomeDirty(target)')!==false);
...
outcomeCheck('dirty state is explicit and save errors keep source-lead edits retryable',strpos($pipelineJs,'Есть несохранённые изменения')!==false&&strpos($css,'.outcomeSaveStatus.dirty')!==false&&substr_count($pipelineJs,"outcomeDirtyLead=target;setOutcomeSaveState('Не удалось сохранить результат','error')")>=2&&strpos($pipelineJs,'function isOutcomeDirty(target=S.current)')!==false);
outcomeCheck('transport failure keeps source-lead outcome edits retryable and surfaces the same error state',strpos($pipelineJs,"}catch(e){if(sameLead(target)){outcomeDirtyLead=target;setOutcomeSaveState('Не удалось сохранить результат','error')}}finally{")!==false&&substr_count($pipelineJs,"setOutcomeSaveState('Не удалось сохранить результат','error')")>=2&&strpos($pipelineJs,'outcomeSavingLeads.delete(target)')!==false);
