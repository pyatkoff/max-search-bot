...
$outcomeJs=(string)file_get_contents($root.'/manager/assets/workspace-v2-outcome.js');
...
outcomeCheck('outcome save is enabled only after a user edit',strpos($leadCardJs,'id="saveOutcome" class="actionBtn primary outcomeSave" type="button" disabled')!==false&&strpos($outcomeJs,'function setOutcomeDirty(dirty=true)')!==false&&strpos($outcomeJs,'button.disabled=!dirty')!==false&&strpos($outcomeJs,'isOutcomeDirty(target)')!==false);
outcomeCheck('all editable outcome fields mark state dirty',strpos($outcomeJs,'outcomeEl.onchange=')!==false&&strpos($outcomeJs,'reasonEl.onchange=')!==false&&strpos($outcomeJs,'noteEl.oninput=')!==false&&substr_count($outcomeJs,'setOutcomeDirty(true)')>=3);
outcomeCheck('dirty state is explicit and save errors keep source-lead edits retryable',strpos($outcomeJs,'Есть несохранённые изменения')!==false&&strpos($css,'.outcomeSaveStatus.dirty')!==false&&substr_count($outcomeJs,"outcomeDirtyLead=target;setOutcomeSaveState('Не удалось сохранить результат','error')")>=2&&strpos($outcomeJs,'function isOutcomeDirty(target=S.current)')!==false);
outcomeCheck('transport failure keeps source-lead outcome edits retryable and surfaces the same error state',strpos($outcomeJs,"}catch(e){if(sameLead(target)){outcomeDirtyLead=target;setOutcomeSaveState('Не удалось сохранить результат','error')}}finally{")!==false&&substr_count($outcomeJs,"setOutcomeSaveState('Не удалось сохранить результат','error')")>=2&&strpos($outcomeJs,'outcomeSavingLeads.delete(target)')!==false);
...
outcomeCheck('sale fields are submitted only through existing outcome boundary',strpos($outcomeJs,'sale_amount:saleAmount')!==false&&strpos($outcomeJs,'sale_date:saleDate')!==false&&strpos($api,"isset(\$data['sale_amount'])")!==false&&strpos($api,"isset(\$data['sale_date'])")!==false);
...
outcomeCheck('sale edits participate in dirty-state protection',strpos($outcomeJs,'saleAmountEl.oninput=()=>setOutcomeDirty(true)')!==false&&strpos($outcomeJs,'saleDateEl.onchange=()=>setOutcomeDirty(true)')!==false);
...
outcomeCheck('outcome persistence has one front-end owner',strpos($outcomeJs,'window.WorkspaceV2Outcome=')!==false&&strpos($pipelineJs,"pipe('set_outcome'")===false&&strpos($pipelineJs,'outcomeDirtyLead')===false);
