...
lcCheck('stage mutation disables control and restores previous value on failure',strpos($js,'const previousStage=')!==false&&strpos($js,'stage.disabled=true')!==false&&strpos($js,'stage.value=previousStage')!==false&&strpos($js,"setSalesSaveState('Не удалось изменить этап','error')")!==false);
lcCheck('stage mutation reports success only after target-pinned refresh',strpos($js,"setSalesSaveState('Этап сохранён','success')")!==false&&strpos($js,'refreshLeadData({refreshInbox:true,conversationId:target})')!==false&&strpos($js,"pipe('set_stage',{conversation_id:target")!==false);
lcCheck('tag mutation guards duplicate save and disables tag controls',strpos($pipeline,'if(tagSaving)return false')!==false&&strpos($pipeline,'inputs.forEach(x=>x.disabled=true)')!==false&&strpos($pipeline,'tagSaving=true')!==false);
...
lcCheck('successful tag mutation reports after refresh',strpos($pipeline,"setSalesSaveState('Теги сохранены','success')")!==false&&strpos($js,'saveTags(x)')!==false);
...
lcCheck('outcome save prevents duplicate submit and reports progress',(strpos($pipeline,'if(outcomeSaving)return')!==false||strpos($pipeline,'if(outcomeSaving||!outcomeDirty)return')!==false)&&strpos($pipeline,"button.textContent='Сохраняем…'")!==false&&strpos($pipeline,"setOutcomeSaveState('Результат сохранён','success')")!==false);
