...
lcCheck('stage mutation disables control and restores previous value on failure',strpos($pipeline,'const previousStage=')!==false&&strpos($pipeline,'stage.disabled=true')!==false&&strpos($pipeline,'stage.value=previousStage')!==false&&strpos($pipeline,"setSalesSaveState('Не удалось изменить этап','error')")!==false);
