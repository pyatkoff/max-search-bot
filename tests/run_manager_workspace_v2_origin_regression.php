...
originCheck('inbox card renders customer plus project and separate origin',strpos($inbox,'c.project_label||c.project_name')!==false&&strpos($inbox,'${esc(name)}${project?` · ${esc(project)}`')!==false&&strpos($inbox,'originText(c)||statusText(c.status)')!==false);
...
originCheck('inbox exports one shared origin projection for other Workspace surfaces',strpos($inbox,'initials,originText,contactOwnerSignals')!==false);
originCheck('kanban card renders customer plus project and separate origin',strpos($kanban,'c.project_label||c.project_name')!==false&&strpos($kanban,'${esc(name)}${project?` · ${esc(project)}`')!==false&&strpos($kanban,'WorkspaceV2Inbox?.originText(c)')!==false);
...
originCheck('lead card service details expose available entry attribution without inventing values',strpos($leadCard,"source.channel||''")!==false&&strpos($leadCard,"source.entry_channel||''")!==false&&strpos($leadCard,"source.attribution_region||''")!==false&&strpos($leadCard,"source.attribution_campaign||''")!==false&&strpos($leadCard,"detailRow('Канал',channel)")!==false&&strpos($leadCard,"detailRow('Точка входа',entryChannel)")!==false&&strpos($leadCard,"detailRow('Регион атрибуции',region)")!==false&&strpos($leadCard,"detailRow('Кампания',campaign)")!==false);
