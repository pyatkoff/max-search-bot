...
shortcutCheck('activating a quick task shortcut switches into manager work queue',strpos($pipeline,"if(next)window.WorkspaceV2Inbox?.setQueue('mine',{reload:false})")!==false);
shortcutCheck('toggling an active shortcut off keeps current queue',strpos($pipeline,"if(next)window.WorkspaceV2Inbox?.setQueue('mine',{reload:false})")!==false&&strpos($pipeline,"if(!next)window.WorkspaceV2Inbox?.setQueue")===false);
shortcutCheck('advanced task select does not force a queue change',strpos($pipeline,"$('leadTaskFilter').onchange=async()=>{S.leadTaskFilter=$('leadTaskFilter').value;await applyFilters()}")!==false);
