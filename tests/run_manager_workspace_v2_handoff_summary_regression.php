...
hsCheck('detail exposes active delivery failure read-only',strpos($api,"require_once \$baseDir.'/services/ManagerDeliveryStateService.php'")!==false&&strpos($api,"'delivery_failure'=>ManagerDeliveryStateService::activeFailure(\$id)")!==false);
...
hsCheck('suspended MAX delivery is explicit',strpos($js,'Клиент недоступен в MAX')!==false&&strpos($js,"failure.notice||failure.message")!==false&&strpos($js,"leadHandoffStrip ${failure?'isError':''}")!==false);
