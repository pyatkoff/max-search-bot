...
$inbox=(string)file_get_contents(dirname(__DIR__).'/services/ManagerLeadInboxService.php');
$passed=0;$failed=0;
...
mw2Check('workspace exposes lead outcome without repurposing technical state',strpos($ui,'id="leadOutcome"')!==false&&strpos($ui,'id="leadCloseReason"')!==false&&strpos($ui,"pipe('set_outcome'")!==false&&strpos($ui,'Технический статус')!==false);
mw2Check('lost outcome requires structured close reason in UI',strpos($ui,"outcome==='lost'&&!closeReason")!==false&&strpos($ui,'Выберите причину отказа')!==false);
mw2Check('workspace has stage tag outcome and search inbox filters',strpos($ui,'id="leadStageFilter"')!==false&&strpos($ui,'id="leadTagFilter"')!==false&&strpos($ui,'id="leadOutcomeFilter"')!==false&&strpos($ui,'id="leadSearch"')!==false&&strpos($ui,'lead_outcome:S.leadOutcomeFilter')!==false&&strpos($ui,'search:S.leadSearch')!==false);
mw2Check('workspace list shows lead-centric business metadata',strpos($ui,'c.lead_stage?.display_name')!==false&&strpos($ui,'c.lead_tags')!==false&&strpos($ui,'c.lead_outcome')!==false&&strpos($ui,'c.trip_summary')!==false&&strpos($ui,'c.unread_count')!==false&&strpos($ui,'c.origin_label')!==false);
...
mw2Check('readonly UI reflects pipeline ownership',strpos($ui,'can_edit_pipeline')!==false&&strpos($ui,'Изменять этап, исход и теги может ответственный менеджер или администратор.')!==false&&strpos($ui,"canEdit?'':'disabled'")!==false);
mw2Check('pipeline API exposes filtered list action',strpos($api,"\$action==='list'")!==false&&strpos($api,"(string)(\$data['lead_stage_key']??'')")!==false&&strpos($api,"(int)(\$data['lead_tag_id']??0)")!==false&&strpos($api,"(string)(\$data['lead_outcome']??'')")!==false&&strpos($api,"(string)(\$data['search']??'')")!==false);
...
mw2Check('V2 inbox projection batches contact outcome and trip summary',strpos($inbox,'SELECT c.id,c.lead_outcome,cu.phone,cu.email')!==false&&strpos($inbox,"text LIKE '%Готово! Проверьте параметры%'")!==false&&strpos($api,'ManagerLeadInboxService::decorate($rows)')!==false);
mw2Check('V2 inbox projection filters synthetic outcome and search behavior',count(ManagerLeadInboxService::filter([['lead_outcome'=>'won','display_name'=>'Анна','trip_summary'=>'Москва Египет'],['lead_outcome'=>'lost','display_name'=>'Борис','trip_summary'=>'Казань Турция']],'won','египет'))===1&&count(ManagerLeadInboxService::filter([['lead_outcome'=>'lost','display_name'=>'Борис','contact_phone'=>'+79991234567']],'','999123'))===1);
