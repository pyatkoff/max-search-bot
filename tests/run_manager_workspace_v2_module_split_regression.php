...
splitCheck('conversation module owns transcript lifecycle and composer',strpos($conversation,'window.WorkspaceV2Conversation=')!==false&&strpos($conversation,'function renderMessages(')!==false&&strpos($conversation,'(messages||[]).forEach')!==false&&strpos($conversation,"change('take')")!==false&&strpos($conversation,"api('send',{conversation_id:S.current,text})")!==false);
