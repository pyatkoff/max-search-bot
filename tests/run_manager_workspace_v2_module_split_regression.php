...
splitCheck('inbox module owns queue-count presentation using existing manager counts API',strpos($inbox,'function refreshQueueCounts')!==false&&strpos($inbox,"api('counts'")!==false&&strpos($inbox,"renderQueueCount('waiting'")!==false&&strpos($inbox,"renderQueueCount('mine'")!==false&&strpos($inbox,"className='filtersCount queueCount'")!==false&&strpos($inbox,'непрочитанных сообщений')!==false&&strpos($api,"if(\$action==='counts')")!==false&&strpos($api,'ManagerConversationService::queueCounts')!==false);
