...
splitCheck('inbox module owns queue-count presentation using canonical manager counts projection',strpos($inbox,'function refreshQueueCounts')!==false&&strpos($inbox,"api('counts'")!==false&&strpos($inbox,"renderQueueCount('waiting'")!==false&&strpos($inbox,"renderQueueCount('requested'")!==false&&strpos($inbox,"renderQueueCount('mine'")!==false&&strpos($inbox,"requested:'Запросили менеджера'")!==false&&strpos($inbox,"className='filtersCount queueCount'")!==false&&strpos($inbox,'непрочитанных сообщений')!==false&&strpos($api,"if(\$action==='counts')")!==false&&strpos($api,'ManagerQueueProjectionService::counts')!==false&&strpos($queueProjection,'class ManagerQueueProjectionService')!==false);
