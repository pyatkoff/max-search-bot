...
/**
 * Bounded AI diagnostics stored outside the public application tree.
 *
 * Raw AI input/context can contain customer conversation data. This logger is
 * deliberately fail-closed when its configured directory resolves inside the
 * repository/document root.
 */
class AiRuntimeLogger
{
    ...
    {
        return self::append('ai_debug.log', $message);
    }

    public static function error(string $message): bool
    {
        return self::append('ai_errors.log', $message);
    }

    public static function debugFile(): string
    {
        return self::file('ai_debug.log');
    }

    public static function errorFile(): string
    {
        return self::file('ai_errors.log');
    }
    ...
    {
        $configured = trim((string)(getenv('MAX_SEARCH_AI_LOG_DIR') ?: ''));
        $dir = $configured !== ''
            ? rtrim($configured, DIRECTORY_SEPARATOR)
            : rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'max-search-bot-ai';
