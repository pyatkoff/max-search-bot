...
class ManagerOutboundService
{
    ...
    public static function send(int $conversationId, int $managerId, string $text): bool
    {
        ...
        if ($channel === 'max') {
            $activeFailure = ManagerDeliveryStateService::activeFailure($conversationId);
            if (is_array($activeFailure)) {
                self::$lastFailure = $activeFailure;
                return false;
            }
    ...
    {
        if (!empty($failure['notice'])) return (string)$failure['notice'];
