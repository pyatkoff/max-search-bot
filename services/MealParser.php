...
class MealParser
{
    ...
    public static function parse(string $text): ?string
    {
        ...
        if (preg_match('/^(?:fb|полный\s+пансион|тр[её]х\s*разов(?:ое|ый|ое\s+питание)|3(?:[\s-]*х)?[\s-]*разов(?:ое|ый|ое\s+питание)|завтрак\s*(?:\+|,|и)\s*обед\s*(?:\+|,|и)\s*ужин|завтрак\s+обед\s+ужин)$/ui', $text)) return 'full_board';
        return null;
    }
}
