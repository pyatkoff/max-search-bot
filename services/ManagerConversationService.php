...
class ManagerConversationService
{
    ...
    public static function list(int $managerId, string $queue='waiting', int $limit=100, string $projectKey='*'): array
    {
        ...
        if($projectKey==='*' || trim($projectKey)===''){
            ...
            if(!$keys)return[];
            $where[]='c.project_key IN ('.implode(',',array_fill(0,count($keys),'?')).')';
            foreach($keys as $key)$args[]=$key;
        }else{
        ...
        if($queue==='waiting'){
            $where[]='c.status=?';$args[]='waiting_manager';
            $where[]='c.manager_id IS NULL';
        }elseif($queue==='mine'){
            $where[]='c.status=?';$args[]='manager';
            $where[]='c.manager_id=?';$args[]=$managerId;
        }elseif($queue==='closed'){
            $where[]='c.status=?';$args[]='closed';
        }else{
            $where[]='c.status<>?';$args[]='closed';
            $where[]='(c.manager_id IS NULL OR c.manager_id=?)';$args[]=$managerId;
        }
        ...
        $sql='SELECT c.id,c.project_key,c.source_id,c.channel,c.status,c.manager_id,c.started_at,c.last_message_at,c.closed_at,'
            ...
            .'LEFT JOIN managers m ON m.id=c.manager_id LEFT JOIN projects p ON p.project_key=c.project_key '
            .'LEFT JOIN conversation_sources s ON s.id=c.source_id WHERE '.implode(' AND ',$where)
    ...
    public static function detail(int $conversationId,int $managerId): ?array
    {
        ...
        $q->execute([$conversationId]);$messages=$q->fetchAll();
        foreach($messages as &$message){if(($message['sender_type']??'')==='manager')$message['text']=html_entity_decode((string)$message['text'],ENT_QUOTES|ENT_HTML5,'UTF-8');}
        unset($message);return['conversation'=>$conversation,'messages'=>$messages];
    }
