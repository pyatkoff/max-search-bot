...
class ManagerAuthService
{
    ...
    {
        self::ensureSchema();$q=ConversationDb::connection()->prepare('SELECT id,login,password_hash,display_name,role,email,is_active FROM managers WHERE login=? LIMIT 1');$q->execute([trim($login)]);$row=$q->fetch();
        if(!$row||!(int)$row['is_active']||empty($row['password_hash'])||!password_verify($password,(string)$row['password_hash']))return null;
        ConversationDb::connection()->prepare('UPDATE managers SET last_login_at=NOW() WHERE id=?')->execute([(int)$row['id']]);unset($row['password_hash']);$row['projects']=ProjectAccessService::projectsForManager((int)$row['id']);return $row;
    }
    public static function byId(int $id):?array
    {
        self::ensureSchema();$q=ConversationDb::connection()->prepare('SELECT id,login,display_name,role,email,is_active FROM managers WHERE id=? LIMIT 1');$q->execute([$id]);$row=$q->fetch();
        if(!$row||!(int)$row['is_active'])return null;$row['projects']=ProjectAccessService::projectsForManager($id);return $row;
    }
}
