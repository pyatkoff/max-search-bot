...
class ManagerAuthService
{
    ...
    public static function authenticate(string $login,string $password):?array
    {
        ...
        ConversationDb::connection()->prepare('UPDATE managers SET last_login_at=NOW() WHERE id=?')->execute([(int)$row['id']]);unset($row['password_hash']);return $row;
    }
    public static function byId(int $id):?array
    {
        self::ensureSchema();$q=ConversationDb::connection()->prepare('SELECT id,login,display_name,email,is_active FROM managers WHERE id=? LIMIT 1');$q->execute([$id]);$row=$q->fetch();return($row&&(int)$row['is_active'])?$row:null;
    }
}
