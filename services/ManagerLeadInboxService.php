...
class ManagerLeadInboxService
{
    ...
    public static function filter(array $rows,string $outcome='',string $search='',string $taskFilter=''): array
    {
        ...
        return array_values(array_filter($rows,static function(array $row)use($outcome,$search,$taskFilter):bool{
            ...
            if($taskFilter==='action'&&(!$hasTask||!in_array($dueState,['overdue','today'],true)))return false;if($taskFilter==='overdue'&&(!$hasTask||!$overdue))return false;if($taskFilter==='today'&&(!$hasTask||$dueState!=='today'))return false;if($taskFilter==='planned'&&(!$hasTask||$dueState!=='upcoming'))return false;if($taskFilter==='pinned'&&(!$hasTask||!$pinned))return false;if($taskFilter==='none'&&$hasTask)return false;
