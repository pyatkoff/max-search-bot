...
- `sw.js`, `push-enable.php`, `push-status.php`, `push.php` — keep; push behavior remains in services while endpoints/pages use the shared HTTP boundary.

### Merge / centralize incrementally

- **Completed:** push endpoints/page, `media-upload.php`, `media-file.php` and `pipeline-api.php` use `ManagerHttp` for their applicable session/auth/CSRF/response lifecycle.
...
2. **In progress:** central request/auth/error interface layer. Push endpoints/page, media upload/preview and Sales Pipeline API are migrated; continue with narrow slices of the remaining endpoints rather than widening `ManagerHttp` into a business layer.
