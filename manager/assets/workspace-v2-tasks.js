(function(){
  ...
  window.WorkspaceV2Tasks={
    ...
    row(task,canEdit){const done=task.status!=='open';return`<label class="taskRow ${done?'done':''}">${canEdit?`<input type="checkbox" data-task-toggle="${Number(task.id)}" ${done?'checked':''}>`:'<span class="taskDot">•</span>'}<span class="taskBody"><span class="taskTitle">${esc(task.title)}</span><span class="taskMeta">${esc(localDue(task.due_at_utc))}${task.assigned_manager_name?` · ${esc(task.assigned_manager_name)}`:''}</span></span></label>`}
  };
})();
