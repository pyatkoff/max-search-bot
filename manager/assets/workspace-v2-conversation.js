...
function applyComposerState(){const suspended=deliverySuspended(),locked=busy||suspended||accessLost,send=$('sendReply'),reply=$('replyText'),file=$('replyFile');if(send){send.disabled=locked;send.textContent=busy?'Отправляем…':'Отправить'}if(reply)reply.disabled=locked;if(file)file.disabled=locked;document.querySelectorAll('.quickReplies [data-reply]').forEach(b=>{b.disabled=locked})}
