...
function pipelineOut(array $d,int $s=200):void{http_response_code($s);echo json_encode($d,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);exit;}
function pipelineBody():array{$v=json_decode((string)file_get_contents('php://input'),true);return is_array($v)?$v:[];}
...
function pipelineRequireCsrf(array $d):void{$e=pipelineCsrf();if($e===''||!hash_equals($e,(string)($d['csrf']??'')))pipelineOut(['ok'=>false,'error'=>'csrf'],403);}
...
function pipelineCanEdit(array $c,array $m):bool{if((string)($m['role']??'manager')==='admin')return true;return(int)($c['manager_id']??0)>0&&(int)($c['manager_id']??0)===(int)($m['id']??0);}
...
$m=pipelineManager();
if(!$m)pipelineOut(['ok'=>false,'error'=>'unauthorized'],401);
pipelineRequireCsrf($data);

if($action==='catalog'){
    pipelineOut([
        'ok'=>true,
        'stages'=>SalesPipelineService::stages(true),
        'tags'=>SalesPipelineService::tags(true),
        'outcomes'=>SalesPipelineService::outcomeOptions(),
        'close_reasons'=>SalesPipelineService::closeReasonOptions(),
    ]);
}
...
if($action==='list'){
    ...
    if(in_array($queue,['waiting','attention'],true)){
        $rows=array_values(array_filter($rows,fn($r)=>empty($r['delivery_failure_category'])));
    }
    $rows=ManagerLeadInboxService::decorate($rows);
    $rows=ManagerLeadInboxService::filter(
        $rows,
        (string)($data['lead_outcome']??''),
        (string)($data['search']??'')
    );
    pipelineOut(['ok'=>true,'conversations'=>array_slice($rows,0,100)]);
}

$id=(int)($data['conversation_id']??0);
$c=pipelineConversation($id,(int)$m['id']);
if(!$c)pipelineOut(['ok'=>false,'error'=>'not_found'],404);
$can=pipelineCanEdit($c,$m);
