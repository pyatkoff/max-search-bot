...
class AiShortAnswerHandler
{
    public static function handle($message, $chat_id)
    {
        ...
        if (!in_array($field, ['adults','children','child_ages','stars','meal','nights'], true)) return false;
        ...
        if ($field === 'adults') {
            ...
            if ($n === null && preg_match('/^(\d)\s*(?:взросл(?:ый|ых|ого)?|человек(?:а)?)$/ui', $lower, $m)) $n = (int)$m[1];
            if ($n === null || $n < 1 || $n > 6) return false;
            $params['adults'] = $n;
        }
        elseif ($field === 'children') {
            $partyClarification = self::partyClarificationWhileAskingChildren($lower);
            if ($partyClarification !== null) $params = $partyClarification;
            elseif (preg_match('/^(?:нет|не будет|без детей|детей нет|без ребёнка|без ребенка|0)$/ui', $lower)) $params['children'] = 0;
            else {
                $n = self::numberFromShortText($lower, 0, 3);
                if ($n === null && preg_match('/^(\d)\s*(?:реб[её]нок|реб[её]нка|реб[её]нков|дет(?:ей|и))$/ui', $lower, $m)) $n = (int)$m[1];
        ...
        elseif ($field === 'child_ages') {
            ...
            foreach ($ages as $age) if ($age < 0 || $age > 17) return false;
        ...
        elseif ($field === 'nights') {
            if (preg_match('/^(?:на\s+)?недел(?:я|ю|ьку)$/ui', $lower)) $params['nights'] = '7';
            else {
        ...
        if (empty($missingAfter)) { DialogueView::check($chat_id); return true; }

        $questions = [
            'city'=>'Из какого города планируете вылет?', 'country'=>'Куда хотите поехать?',
            'adults'=>'Сколько будет взрослых туристов?', 'children'=>'Будут дети? Если да — сколько?',
            'child_ages'=>'Сколько лет детям?', 'stars'=>'Какая минимальная категория отеля нужна — 3, 4 или 5 звёзд?',
            'meal'=>'Какое питание предпочитаете?', 'nights'=>'На сколько ночей планируете поездку?',
            'date'=>'Какая ориентировочная дата вылета?'
        ];
        MaxSearchApi::setStatus($chat_id, MaxSearchApi::$statusAi);
        IntegrationRegistry::messenger()->send($chat_id, $questions[$missingAfter[0]] ?? 'Уточните, пожалуйста, параметры поездки.');
        return true;
    }
    ...
    public static function partyClarificationWhileAskingChildren(string $text): ?array
    {
        ...
        $lower = trim(preg_replace('/[.!?]+$/u', '', $lower));
        if (!preg_match('/^([1-6])\s*(?:взросл(?:ый|ая|ые|ых|ого))$/ui', $lower, $m)) return null;
        return ['adults'=>(int)$m[1], 'children'=>0];
    }
    ...
    public static function starMinimumFromShortText(string $text): ?int
    {
        $lower = function_exists('mb_strtolower') ? mb_strtolower(trim($text), 'UTF-8') : strtolower(trim($text));
        $lower = trim(preg_replace('/[.!?]+$/u', '', $lower));
        if (preg_match('/^(?:не важно|неважно|любая|любые|все|всё)$/ui', $lower)) return 1;
        if (preg_match('/^(?:от\s*)?([1-5])\s*(?:\*|★|зв[её]зд(?:а|ы)?)?$/ui', $lower, $m)) return (int)$m[1];

        // Live users often answer the minimum-category question with a set like "3,4" or "3-4".
        // The search model stores one minimum star value, so the least category in an explicit valid set is the faithful value.
    ...
    {
        if (preg_match('/^\d+$/', $text)) { $n=(int)$text; return ($n >= $min && $n <= $max) ? $n : null; }
        $words = [
            'ноль'=>0, 'один'=>1, 'одна'=>1, 'одного'=>1,
            'два'=>2, 'двое'=>2, 'двух'=>2, 'три'=>3, 'трое'=>3, 'трех'=>3, 'трёх'=>3,
            'четыре'=>4, 'четверо'=>4, 'пять'=>5, 'пятеро'=>5, 'шесть'=>6, 'шестеро'=>6,
        ];
        if (!array_key_exists($text, $words)) return null;
        $n=$words[$text]; return ($n >= $min && $n <= $max) ? $n : null;
    }
}
