...
require_once dirname(__DIR__) . '/services/WebhookTargetConfig.php';
...
final class MaxWebhookHealth
{
    ...
    public static function collect(?callable $request = null): array
    {
        ...
        $expected = WebhookTargetConfig::max();
        if ($token === '') {
    ...
    public static function evaluate(array $response, string $expectedUrl): array
    {
        ...
        $reason = $ok ? 'healthy' : ($expectedPresent ? 'extra_subscriptions' : 'expected_subscription_missing');
